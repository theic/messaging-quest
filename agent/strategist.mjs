...
import { memoryContext, readOne, writeMemory } from "../lib/memory.mjs";
...
import { isCustomer } from "../lib/chat.mjs";
...
import { QUEST_HEARTBEAT_ASK, QUEST_MAIL_ASK, questPromptOf, questPreface, questTools, notebook, lastReadAt, openRouterFetch, contentText } from "./quest.mjs";

export { questTools } from "./quest.mjs";
 ...
 *  hash all live down there, which is exactly why. A customer's project gets
 *  Quest's much shorter list instead (questTools, below). */
const makeTools = (dir) => (isCustomer(dir) ? questTools(dir) : operatorTools(dir));

const operatorTools = (dir) => [
  ...engineTools(dir),
...
export async function turnPreface(dir, { wokeBy = null } = {}) {
  if (isCustomer(dir)) return questPreface(dir, { wokeBy, lane: runtimeOf(dir).control });
...
export function systemPromptOf(dir) {
  if (isCustomer(dir)) return questPromptOf(dir);
...
async function agentFor(dir) {
  ...
  // A skill's subagent is a colleague of the operator's; Quest works alone.
  const subagents = isCustomer(dir) ? [] : await skillSubagents(dir);
...
/** Which thread a turn lands on when the caller does not say: a customer's
 *  project has ONE conversation — the chat, the heartbeat and the inbox are
 *  all the same Quest remembering the same things — and the operator's panel
 *  keeps its own. */
const threadOf = (dir, thread) => thread ?? (isCustomer(dir) ? "chat" : "panel");
...
export function startInboxLoop(dir, {
  everyMs = 20_000, thread = null, heartbeatMs = HEARTBEAT_MS, ceilingMs = HEARTBEAT_CEILING_MS,
  ...
  // operator's money is spent. The default is the real thing — on the
  // project's own thread, asked per turn (threadOf), so a project that
  // became a customer's after the loop started lands in the customer's chat.
  speak = (content, opts) => serial(dir, () => turn(dir, content, threadOf(dir, thread), opts)),
} = {}) {
  let backoffUntil = 0;
  let inFlight = false;
  ...
  const heartbeat = async () => {
    ...
    try {
      const reply = await speak(isCustomer(dir) ? QUEST_HEARTBEAT_ASK : HEARTBEAT_ASK, { wokeBy: "your heartbeat — nobody asked for it" });
  ...
  const pulse = async () => {
    ...
    try {
      ...
      const ask = isCustomer(dir) ? QUEST_MAIL_ASK : dayDigest
