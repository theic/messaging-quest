...
// reads Reddit. The verbs are still one implementation (lib/verbs.mjs) — the
// buttons run them in this process (lib/jobs.mjs start()) rather than
// reimplementing them.
//
// From 0.10.0 this file is the dashboard only: the HTML views, the form
// writes, the static panel, the brain when agent/ is installed. The deck, the
// jobs, the panel's tabs, the control lane and the JSON routes over them are
// lib/engine.mjs — the same engine the extension's worker runs with no server
// at all. What is served here at /api/* is that engine's one route table.
//
...
/** The data directory — the root project, the registry beside it, and
 *  the machine's own files (lib/dirs.mjs). The engine (lib/engine.mjs) is
 *  built on it once the platforms are loaded; the views below take the
...
const writes = {
  ...
  "/setup/scout": (form) => {
    ...
    const started = J.run("scout", async (ctl) => {
      ...
      return await scoutSite(P(), url, ctl, { browse: browser(LANE, { task: "reading your site", project: P() }) });
  ...
  "/api/run": (form) => {
    ...
    J.start(verb, args, { label: SPAWNABLE[verb] });
...
const server = createServer((req, res) => {
  ...
  /* The engine's JSON API (lib/engine.mjs): the deck, the projects, the
   * panel's tabs, the control lane, the strategist, the jobs — one table of
   * routes, served here over HTTP and in the extension's worker over
   * messages. A POST must declare application/json (jsonBody): that is the
   * CSRF boundary. */
  if (E.routes(req.method, url.pathname)) {
    const q = Object.fromEntries(url.searchParams);
    const answer = (out) => res.writeHead(out.status, JSON_HEAD).end(JSON.stringify(out.body));
    if (req.method === "POST")
      return jsonBody(req).then((body) => E.handle(req.method, url.pathname, q, body)).then(answer)
        .catch((e) => res.writeHead(400, JSON_HEAD).end(JSON.stringify({ error: e.message })));
    return E.handle(req.method, url.pathname, q, null).then(answer);
  }
