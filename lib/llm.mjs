...
export async function complete(seat, messages, opts = {}) {
  const body = { model: seat.model, messages, temperature: 0, max_tokens: seat.maxTokens };
  // OpenRouter's own fields. A local server either refuses a field it does
  // not know or — worse — ignores it, so a fallback list that does nothing
  // looks like one that works. They go only where they mean something.
  if (seat.openrouter) {
    if (seat.models?.length) body.models = seat.models;
    body.provider = { sort: "throughput", allow_fallbacks: true };
    // OpenRouter's plugins — the file parser that turns an attached PDF into
    // text before any model sees it (a customer's CV, Stage 1 of the Quest
    // plan). Nowhere else: a local server has no such thing to ask.
...
async function post(seat, body, signal) {
  ...
  try {
    const res = await fetch(`${seat.baseUrl}/chat/completions`, {
      method: "POST",
      // No key, no header: Ollama wants none, and sending "Bearer null" to a
      // server that does check would fail for the wrong reason.
      headers: { "content-type": "application/json", ...(seat.key ? { authorization: `Bearer ${seat.key}` } : {}) },
 ...
 */
export async function structured(seat, messages, schema, { name = "answer", signal, log, plugins } = {}) {
  ...
  for (let attempt = 0; ; attempt++) {
    const { message, usage } = await complete(seat, convo, { tools, tool_choice, signal, plugins });
    const raw = message?.tool_calls?.[0]?.function?.arguments;
    let data, why;
    if (raw === undefined) {
      // Some local servers accept `tool_choice` and quietly ignore it; the
      // model then answers with the JSON in prose. A conforming object in that
      // prose IS the answer — the same validator checks it, so this is not a
      // looser bar, only a second place to look.
      const found = salvage(message?.content, schema);
      if (found) { log?.(`  ${name} arrived as prose with the JSON inside — taken`); return { data: found, usage }; }
      why = `the model answered in prose instead of calling ${name}`;
    }
...
/**
 * The JSON object inside a prose answer, if one conforms to the schema. Tries
 * the whole text, then a fenced block, then the outermost braces — which also
 * skips a local reasoning model's <think> preamble. Null when nothing fits.
 */
export function salvage(text, schema) {
  const s = typeof text === "string" ? text : Array.isArray(text) ? text.map((p) => p?.text ?? "").join("") : "";
  ...
  for (const t of tries) {
    try {
      ...
      if (v && typeof v === "object" && !conforms(schema, v)) return v;
    } catch { /* not this one */ }
  }
  return null;
}
