...
import { existsSync, mkdirSync, readFileSync, writeFileSync, copyFileSync, join } from "./fs.mjs";
 ...
 *
 *  `inherit: false` is a CUSTOMER's project (lib/chat.mjs): somebody else
 *  entirely, so the operator's account, voice, me.md and persona stay out of
 *  it — a draft written for a customer must never carry the operator's own
 *  claims about themselves. */
export function createProject(root, name, { rename = false, inherit = true } = {}) {
  const id = slug(name);
  ...
  seedMissing(dir);
  for (const f of inherit ? INHERITED : []) {
