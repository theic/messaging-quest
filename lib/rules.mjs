...
// Where a platform keeps its rules, and which of its communities are jokes,
// are the platform's to say (skills/<id>/adapter.mjs: rulesUrl, parody).
// This file stopped carrying Reddit's on 2026-09-04.
 ...
 *  `label` is what the platform's people write ("r/saas"); `rulesUrl` is
 *  where a person reads the rules, when the platform has such a page. */
export const roomFile = (place, found, { label = String(place), rulesUrl = null } = {}) => `# ${label}

## Do the rules permit promotion here?

promotion_allowed: UNANSWERED

The platform does not show a room's rules to a logged-out reader, and the
public description is not the rules list — measured, and the reason this line
exists rather than being guessed at.

**Read them once:** ${rulesUrl ?? "on the room's own rules page"}

Then set the line above to \`yes\` or \`no\`. Until it says something, this room
cannot be watched — \`mq watch\` refuses it, deliberately.

${found ? `> The public description already says: "${found}"\n>\n> That alone is enough to answer \`no\`.\n` : ""}
## Your standing here

comments: unknown — \`mq ready\` measures this once you have history
`;

/**
 * What a room's rules page, read whole, says about promotion: `banned` with
 * the sentence that forbids it, or `allowed`. A page whose text has no full
 * stop before the rules list gives back a "sentence" that starts at the page
 * furniture ("Skip to main content Open navigation…"), so a long one keeps
 * its last 240 characters. Asked wherever that page was read — the engine's
 * own browser, or the operator's on a job the cloud handed it.
 */
export function rulesVerdict(pageText) {
  const found = bansPromotion(pageText);
  const quote = found && found.length > 240 ? `…${found.slice(-240).replace(/^\S*\s/, "")}` : found;
  return { state: quote ? "banned" : "allowed", quote: quote ?? null };
}

/** The room file once its rules page was read: the answer line set, and a
 *  dated note saying what the page said — the same file a human answers. */
export const roomRead = (base, verdict, when) =>
  base.replace(/^promotion_allowed:.*$/mi, `promotion_allowed: ${verdict.quote ? "no" : "yes"}`)
  + `\n## Read by Messaging Quest, ${when}\n\n${verdict.quote ? `Its rules say: "${verdict.quote}"` : "Nothing on its rules page forbids promotion. Correct the line above if a human reading them disagrees."}\n`;
