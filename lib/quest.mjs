...
export function offerFrom(p = {}) {
  ...
  return {
    ...
    // What the communities where these people talk are ABOUT — searched for
    // on the platform, before the card, to find rooms that exist.
    topics: searchesOf(p.topics).slice(0, 3),
...
/**
 * The communities a platform's own search turned up for the offer's topics —
 * one list a topic (`lists`: what the adapter's discoverRooms read off each
 * search page, in the platform's ranking) — as one: each topic's best first
 * (round-robin, so one broad topic cannot crowd the others out), a room once
 * whatever its spelling, without profiles and the platform's own joke
 * communities (`parody`), and without the ones nobody is in — a community
 * with a thousand weekly visitors and fifty contributions is somewhere a post
 * is read; one with eighteen and none is not (measured 2026-09-19). The quiet
 * ones only fill up to what the first look needs, busiest first. A number the
 * page did not say is not held against a room.
 *
 * This exists because the offer's own list of communities was wrong five
 * times in five (2026-09-19: names a model composed from the topic — a topic
 * word and "forums" — none of which had a post to read). A room a platform's
 * search lists exists.
 */
export function discoveredPlaces(lists = [], { parody = () => false, minVisitors = 1000, minContributions = 50, keep = FIRST_LOOK } = {}) {
  const seen = new Set();
  const all = [];
  const longest = Math.max(0, ...arr(lists).map((l) => arr(l).length));
  for (let i = 0; i < longest; i++) {
    for (const l of arr(lists)) {
      const c = arr(l)[i];
      const place = bare(c?.place ?? "");
      const k = place.toLowerCase();
      if (!place || seen.has(k) || /^u_/i.test(place) || parody(place)) continue;
      seen.add(k);
      all.push({ ...c, place });
    }
  }
  const alive = (c) => (c.visitors ?? minVisitors) >= minVisitors && (c.contributions ?? minContributions) >= minContributions;
  const live = all.filter(alive);
  const quiet = all.filter((c) => !alive(c)).sort((a, b) => (b.contributions ?? 0) - (a.contributions ?? 0));
  return [...live, ...(live.length < keep ? quiet.slice(0, keep - live.length) : [])];
}

/**
 * The communities an offer names: the rooms real posts turned up in first
 * (`found`, from discoveredPlaces), and the model's own guesses only to fill
 * what the first look needs — never beside a full list of real ones, because
 * a guess costs a rules read and a search of a room that may not exist.
 */
export function placesFrom(found = [], guessed = [], { keep = FIRST_LOOK, max = 6 } = {}) {
  const real = uniqCI(arr(found).map((f) => bare(f?.place ?? f)).filter(Boolean));
  const seen = new Set(real.map((p) => p.toLowerCase()));
  const fill = real.length >= keep ? [] : uniqCI(arr(guessed).map(bare).filter((g) => g && !seen.has(g.toLowerCase()))).slice(0, keep - real.length);
  return [...real, ...fill].slice(0, max);
}

/**
 * What Quest says to a customer's message when the model had nothing to say —
 * a free model answered a first message with "noted", three times of three
 * (2026-09-19), and that word, which the heartbeat uses for "nothing to add",
 * left a person who had just typed their address with a silent chat. Someone
 * who wrote is owed an answer; this is one that is always true.
 */
export function ackFor({ host = null, offer = null, reading = false } = {}) {
  if (reading && host) return `Got it — I'm reading ${host} now. What I understood will land here as a card for you to check.`;
  if (reading) return "Got it — I'm reading it now. What I understood will land here as a card for you to check.";
  if (offer?.state === "confirmed") return "Got it — I'm on it, and I'll post what I find right here.";
  if (offer) return "Got it. The card above has what I understood — tell me if anything is off, or press Looks right.";
  return "Got it.";
}
