...
import { existsSync, mkdirSync, readdirSync, readFileSync, writeFileSync, join, basename, sha256 } from "./fs.mjs";
...
export const campaignHash = (c) =>
  sha256([c.mention ?? "never", c.idea ?? "", c.fit ?? "", c.never ?? "", c.voice ?? ""].join("\n\x00")).slice(0, 8);
