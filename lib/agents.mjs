...
const readUrlTool = (ctl, seen, browse) => ({
  name: "read_url",
  description: "Open one public web page in the operator's browser and return its text and the links on it. Reads only.",
  parameters: {
    type: "object",
    required: ["url"],
  ...
  run: async ({ url }) => {
    ...
    if (opened.error) return `could not open it: ${opened.error}`;
    const page = await browse.text(PAGE_LIMIT);
    if (page.error) return `could not read it: ${page.error}`;
...
export async function scoutSite(dir, url, ctl = {}, { browse } = {}) {
  ...
  try {
    ({ text: prose, usage, exhausted } = await toolLoop(cfg, {
      ...
      tools: [readUrlTool(ctl, seen, browse)],
...
const OPTIONS = {
  ...
  properties: {
    no_fit: {
      type: "string",
      description: "Only when there is nothing honest to say without naming what the operator sells: one plain sentence on why. Leave options empty then.",
    },
    options: {
      type: "array",
      description: "Up to three, differing by move. One is a correct answer if there is only one honest thing to say; none, with no_fit filled in, when the gate failed.",
      items: {
        type: "object",
        required: ["move", "text"],
        properties: {
 ...
 *
 * Returns `{options, no_fit}`: an empty list with a reason is the writer
...
export async function draftReply(dir, promptText, ctl = {}) {
  ...
  const { data } = await structured(cfg, [
    { role: "system", content: "You are the writer. Everything you need is in the next message. Answer only by calling the options function — the replies go in its arguments, and nothing goes outside it." },
    { role: "user", content: promptText },
  ], OPTIONS, { name: "options", signal: ctl.signal, log: ctl.log });
  // A writer that fills no_fit AND options has misread the field; the
  // options are the answer and the reason is noise.
  const options = data.options ?? [];
  return { options, no_fit: options.length ? null : (data.no_fit ?? null) };
}
