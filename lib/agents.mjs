...
is spent. Every page you open is a real page turn in somebody's browser, so
never open the same page twice under another address, and leave sign-in and
sign-up pages alone. Legal pages (terms, privacy, cookies, refunds,
acceptable use) are the company's rules, not its buyers: read one only when
no other page says what the product is, and never turn what they forbid into
who to answer. If the site does not say what it sells (a holding page, "under
construction"), say exactly that rather than filling the gaps.
...
/** What the proposal is ABOUT. Measured 2026-09-18 on a free model reading a
 *  repository's README: it wrote the offer from the OWNER's side — signals
 *  like "ask about my product's features", a rule that says YES when a post
 *  "mentions the product name" — which finds nobody, because nobody who needs
 *  a product has heard of it. The people to find are its buyers, in their
 *  words, and both readers (the site's and the CV's) are told so. */
export const BUYER_RULES = `The people to find are BUYERS: people who have the problem this offer solves and have probably never heard of it. Write every part about them, from their side of the screen:
- "problem" and "signals" are what a stranger writes or does in a public post. Never about this product: no "my product", "my tool" or "my service", no product name, no questions about its features, docs, pricing or setup — that is what its USERS write, and they are not who to find.
- "searches" are what that stranger would type to find help: two to four plain words for their problem or need ("first customers", "no audience"). Never the product's name, and never a category word on its own ("alternative to", "how to").
- rule_md is about the person who wrote the post: answer YES when what they say shows they have this problem or need this kind of help. A post that names the product is not the target.
- "places" are communities people join to ask each other for help — not apps, networks or chat services.`;

/** The second step of the site read: the findings, as the three files. */
export const PROPOSAL_BRIEF = `Turn these findings into the files. ${BUYER_RULES}

Keep the person's own words wherever you can. Never invent a fact the findings do not contain.`;

const PROPOSAL = {
  type: "object",
  required: ["name", "one_line", "problem", "project_md", "icp_md", "rule_md", "places", "unknown"],
  properties: {
    name: { type: "string", description: "What the company or product is called, exactly as the material names it. If the material does not name it, an empty string — never make one up: it is put in the customer's mouth ('I built …')." },
    one_line: { type: "string", description: "What it is and who buys it, in one plain sentence under 20 words — like 'A scheduling tool for independent consultants.'" },
    problem: { type: "string", description: "The problem in the words of somebody who has it: one short sentence under 20 words, the way they would write it in a post." },
    // What the customer is shown ("I'll look for people who: …") and what is
    // typed into a community's search box — short, or the search finds nothing.
    signals: {
      type: "array", items: { type: "string" },
      description: "3 to 5 things a stranger does in a post when they have the problem this offer solves, each under 10 words and starting with a verb — 'ask for a scheduling tool', 'complain about chasing invoices', 'look for a Calendly alternative'. About them, never about this product.",
    },
    searches: {
      type: "array", items: { type: "string" },
      description: "2 to 4 phrases of 2 to 4 plain words to type into a community's search box to find those posts — 'calendly alternative', 'scheduling tool'. No quotes, no operators.",
    },
    // Not required: a model that leaves it out costs the offer its search for
    // real communities, not the offer.
    topics: {
      type: "array", items: { type: "string" },
      description: "1 to 3 broad topics, one or two plain words each, that the communities where these people talk are ABOUT — 'language learning', 'expats', 'freelancing'. What a person would type into a search box to find those communities. A topic: never a community name, never this product.",
    },
    ...
    rule_md: {
      ...
      description: "Full markdown for rule.md: a fit rule with an 'Answer YES when' list, a 'The near miss' " +
        "list, and a 'When you cannot tell' section that says to answer YES. It is about the person writing " +
        "a post — what they describe, ask for or complain about — never the company's own terms or policies.",
    },
    places: { type: "array", items: { type: "string" }, description: "Community names, without a platform prefix, where people with this problem ask each other for help — communities, not apps, networks or chat services. Only names you are sure exist and are large and well known, spelled exactly as they are written after the prefix. A name you would have to compose from the topic (a topic word plus 'forums', 'groups' or 'communities') does not exist: leave it out. Fewer is better than a guess. At most six." },
...
export async function scoutSite(dir, url, ctl = {}, { browse } = {}) {
  ...
  if (!stats.ok) throw new Error(`could not read ${site} — ${stats.lastError ?? "no page opened"}. ${/allow|grant/i.test(stats.lastError ?? "") ? "Press Allow on the panel's card for the site, then try again." : /on screen|covered|minimi[sz]ed/i.test(stats.lastError ?? "") ? "Bring Chrome's window into view, then try again." : "Check the address, then try again."}`);

  ctl.progress?.(2, 3, "writing the files");
  ctl.log?.("\nturning that into project.md, icp.md and rule.md");

  const { data } = await structured(cfg, [
    { role: "system", content: PROPOSAL_BRIEF },
...
/** The scout's brief, for pages somebody else's browser already turned
 *  (Stage 2 of the Quest plan): on Supabase the operator's Chrome reads the
 *  site as a whole job (lib/reads.mjs readSite) and hands the pages back, and
 *  the choosing that is left is what they say. */
const SITE_BRIEF = `You are given the pages of one company's own website, as a person's browser read them: the page they gave, and the pages it links to that say more. A marketing tool will use what you write to tell, later and automatically, whether a stranger's post is somebody that company should answer.

What you are looking for, in order of importance:
1. The PROBLEM somebody has before they find this company. In their words, not the company's. The sentence a person types into a forum at 1am.
2. What the thing actually does. Concretely. Not the tagline.
3. Who it is for, and — harder and more valuable — who looks like a match and is NOT.
4. What a stranger can go and check. Named customers, public numbers, a free tier, a repo.

Legal pages (terms, privacy, cookies, refunds, acceptable use) are the company's rules, not its buyers: never turn what they forbid into who to answer. If the pages do not say what it sells (a holding page, "under construction", a login wall), say exactly that — "Unknown" for what it is — rather than filling the gaps. Do not guess.`;

/**
 * The offer from a site whose pages were read elsewhere: one structured call,
 * the same PROPOSAL the scout writes, from `pages` = [{ url, title, text }].
 * Writes nothing, like the scout.
 */
export async function offerFromPages(dir, url, pages, ctl = {}) {
  const cfg = seat(dir, "scout");
  const read = (Array.isArray(pages) ? pages : []).filter((p) => String(p?.text ?? "").trim());
  if (!read.length) throw new Error(`nothing of ${url} could be read`);
  ctl.log?.(`offer: ${cfg.model}, ${read.length} page${read.length === 1 ? "" : "s"}`);
  const body = read.map((p) => `URL: ${p.url}${p.title ? `\nTitle: ${p.title}` : ""}\n\n${String(p.text).slice(0, 12_000)}`).join("\n\n---\n\n").slice(0, 40_000);
  const { data } = await structured(cfg, [
...
/* ------------------------------------------- an instruction, in their words */

const INSTRUCTION = {
  type: "object",
  required: ["leave_out", "places", "also_asks"],
  properties: {
    leave_out: {
      type: "array", items: { type: "string" },
      description: "Kinds of people they ask you to STOP finding, each a short noun phrase in their words — 'agencies', 'people selling services'. If they narrow it ('only people who built their own product'), write what that excludes ('people who did not build their own product'). Empty when they did not ask to change who to find: a question about why somebody was included is not a change.",
    },
    places: {
      type: "array", items: { type: "string" },
      description: "Communities they ask you to ADD, bare names without a platform prefix — only when they name one and ask for it. Empty otherwise.",
    },
    also_asks: { type: "boolean", description: "True when the message ALSO asks a question or wants something else done — a reply rewritten, something explained — beyond these changes." },
  },
};

const INSTRUCTION_BRIEF = `A customer is talking to the assistant that finds people asking for what they sell. Read their message and pull out ONLY what they ask to change about WHO gets found: kinds of people to leave out, and communities to add. A question, a compliment, a complaint about one reply or a request to rewrite a reply changes nothing here — return empty lists for those. Use their own words; never invent.`;

/**
 * What a customer's message asks to change about who is found — read by a
 * forced structured call, which a free model conforms to reliably, rather
 * than by hoping the chat model remembers to call a tool (lib/revise.mjs says
 * why). Returns the raw fields; instructionFrom (lib/quest.mjs) cleans them.
 */
export async function interpretInstruction(dir, text, ctl = {}) {
  const cfg = seat(dir, "judge");
  const { data } = await structured(cfg, [
    { role: "system", content: INSTRUCTION_BRIEF },
    { role: "user", content: `Their message:\n\n${String(text ?? "").slice(0, 2000)}` },
  ], INSTRUCTION, { name: "instruction", signal: ctl.signal, log: ctl.log });
  return data;
}

/* ------------------------------------------------------ from a CV or words */

const MATERIAL_BRIEF = `You are given what one person sells, in their own material: a CV, or a few sentences describing a product or a service. Nothing else — no website.

Work out, from that material only:
1. What they sell. For a CV, that is their services: the work they are hired to do, at the level the CV supports.
2. Who buys it — and, harder and more valuable, who looks like a buyer and is NOT. For a CV the buyers are the founders, teams or businesses who need this work done now, not recruiters and not other freelancers.
3. The sentence a buyer types in a forum when they need it ("looking for someone to…", "how do I…").
4. Online communities, without a platform prefix, where those buyers ask for it. Best guess, at most six; prefer the ones where people post needs, not the ones where sellers advertise.

Never invent a skill, a client, a number or a product the material does not state — and never a name: if the material does not name what they sell, "name" is empty. What it does not say goes in "unknown".

${BUYER_RULES}`;

/**
 * The offer from what the customer handed over instead of a site (Stage 1 of
 * the Quest plan): a CV as a PDF, or their own sentences. No browser — the
 * material is already here. A PDF reaches the model through OpenRouter's file
 * parser, which turns it into text first; a local model is asked to take the
 * text instead, in words it can act on.
 */
export async function offerFromMaterial(dir, { text = "", file = null } = {}, ctl = {}) {
  const cfg = seat(dir, "scout");
  const words = String(text ?? "").trim().slice(0, 12_000);
  if (!words && !file) throw new Error("nothing to read — no CV and no description");
  if (file && !cfg.openrouter) throw new Error("reading a PDF needs the OpenRouter plan — paste the CV's text into the chat instead");
  ctl.progress?.(0, 2, file ? "reading your CV" : "reading what you sell");
  const content = [
    { type: "text", text: `${file ? `Their CV is attached (${file.name}).` : "Their own words:"}\n\n${words || "(no other words — the CV is the whole of it)"}` },
    ...(file ? [{ type: "file", file: { filename: String(file.name ?? "cv.pdf").slice(0, 120), file_data: file.data } }] : []),
  ];
  const { data } = await structured(cfg, [
    { role: "system", content: MATERIAL_BRIEF },
    { role: "user", content },
  ], PROPOSAL, { name: "proposal", signal: ctl.signal, log: ctl.log, plugins: file ? [{ id: "file-parser", pdf: { engine: "pdf-text" } }] : undefined });
  ctl.progress?.(2, 2, "done");
  return data;
}
