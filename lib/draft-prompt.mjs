...
import { communityRisks, signalWritingRules } from "./writing.mjs";
/**
 ...
 */
/**
 * Replies one run may write.
 *
 * Three, and it is a ceiling rather than a target — the prompt says so out
 * loud. The reason the number exists at all is that choosing between real
 ...
 * going to make anyway. Edit distance measured how much they retyped, which
 * across 46 drafted leads was recorded twice.
 */
...
WRITE ${input.instruction ? "ONE revision" : `UP TO ${DRAFT_OPTIONS} REPLIES, and they must differ in APPROACH rather than in phrasing`}. ${input.instruction
        ? ""
        : `Three ways of saying one sentence is a worse product than one good reply, and the user can tell instantly. Different approaches means: answering the literal question versus answering what is behind it; leading with the specific detail versus leading with the shared experience; solving it outright versus pointing them at whoever already solved it. If there is only ONE honest thing to say here, send one option — that is a correct answer and a better one than padding.

Each option carries a SHORT NAME you write, one or two words, naming what actually differs about it: "direct", "story first", "just the link", "asks back". Never "option 2", never a name that describes the tone in general.`}

Write a file draft.json:
  {"options": [{"label": "direct", "message": "the reply, under ${lengthCeiling(input.voice ?? null)} characters"}${input.instruction ? "" : ', {"label": "story first", "message": "…"}'}],
   "why": "${talking ? "what they asked, and what this answers" : "they said X, and <what the user built> fixes that"}"}
The "why" describes the LEAD, not the options, so there is one of it whatever you send.${talking
        ? ""
