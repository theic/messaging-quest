...
cmds.probe = async (args) => {
  ...
  if (!r.ok) { await b.close(); return log(`  could not read it: ${r.error}\n  That is a failed read, not a verdict on the room.`); }
  ...
  log(`  ${r.entries.length} posts on the page, ${fresh.length} new to judge${bodies < fresh.length ? ` (${fresh.length - bodies} previewed only — the page showed no body and the ${P().bodiesPerRead}-per-read cap on opening posts was spent)` : ""}.`);
...
cmds.tick = async (args) => {
  ...
  const due = sources().filter((s) => {
    const r = lr.get(s.id);
    // Never read as itself: the probe that earned the watch read this very
    // page moments before it was added, so it is first due one cadence after
    // that (2026-09-18: a fresh watch was read again a minute later). A source
    // with neither — added by hand, before `added` existed — is due now.
    const last = r ? Date.parse(r.at) : Date.parse(s.added ?? "");
    return !Number.isFinite(last) || Date.now() - last >= s.cadence_min * 60_000;
  }).slice(0, limit);
  ...
  for (const s of due) {
    ...
    if (!r.ok) { log(`  ${s.id}: ${r.error}`); if (/not attached|no server/i.test(r.error)) { dark = true; break; } continue; }
    const seen = new Set();
    const fresh0 = r.entries.filter((e) => e.kind === "post" && !known.has(e.id) && !seen.has(e.id) && seen.add(e.id)
    ...
    log(`  ${s.id}: ${r.entries.length} read, ${fresh} new`);
  }
  if (b) await b.close();
  setPending(p);
...
cmds.serve = async (args) => {
  const { serve } = await import("../bin/serve.mjs");
