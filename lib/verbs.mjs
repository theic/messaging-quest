// The verbs — one implementation of every one, as a factory.
//
// This was the body of bin/mq.mjs until 0.10.0. The CLI is still one caller
// (argv in, stdin in, lines out); the dashboard and the extension's worker
// are the others, and they call the same functions in-process with a `log`
// of their own instead of spawning `node bin/mq.mjs <verb>` — the hosted
// product has no Node to spawn, and two implementations of `probe` is how
// a queue starts disagreeing with itself.
//
//   verbs({ root, dir, log, warn, browse })  →  { init, me, sync, …, project }
//
// `root` is the data root (the default project, the registry, the machine's
// files) and `dir` the project every verb acts on — lib/projects.mjs. `log`
// takes a line the way console.log did; `warn` a line for stderr. `browse`
// makes the browser a read goes through (lib/browse.mjs): the CLI's default
// reaches the server that holds the control lane; the worker passes its own
// broker. A verb that cannot go on throws VerbError with the sentence a
// person reads — the CLI prints it as `mq: …` and exits 1; a job runner
// records it as the job's failure.
//
// The rest of the file is the CLI's original prose and code, unchanged
// except for those seams; read bin/mq.mjs's header for what it is for.

import { existsSync, mkdirSync, readFileSync, writeFileSync, appendFileSync, join, sha256, env } from "./fs.mjs";
import { loadPlatforms, platforms, first, preferred, platformFor, labelsOf, accountOf, whoamiOf } from "./platform.mjs";
import { skillState, writeChoice } from "./skills.mjs";
import { PLANS, ROLES, plan, setPlan, chosen, choose, localConfig, setLocal, probeLocal, hasKey, keySource } from "./models.mjs";
import { classify, history, STATES } from "./verdict.mjs";
import { conversation, byUrgency } from "./conversation.mjs";
import { conversationRows, openConversation, bindConversations, recordReturn, waiting as waitingRows, yourTurns, dueConversations, unbound, campaignDigest, digestText } from "./conversations.mjs";
import { readStash, patchStash } from "./cards.mjs";
import { verdictOf } from "./probe.mjs";
import { fromDescription, roomFile } from "./rules.mjs";
import { store, FILES, dataDir } from "./store.mjs";
import { browser, serverBase } from "./browse.mjs";
import { readCampaigns, readCampaign, setCampaignStatus, writerBlock } from "./campaigns.mjs";
import { listProjects, createProject, useProject, currentDir } from "./projects.mjs";
import { seedMissing, readOne } from "./memory.mjs";
import { measureVoice, mergeVoice, voiceRules, voiceSummary, lengthCeiling, MIN_SAMPLE_CHARS } from "./voice.mjs";
import { signalWritingRules, communityRisks, draftsBlock, styleOf, styleLabel, YOURS } from "./writing.mjs";
import { repeats, claims, inventedLinks, tells, theirs, RUN_LIMIT } from "./guards.mjs";
import { standing, readiness, burst, mix, CQS_NOTE, PER_ROOM_24H, OVERALL_24H } from "./ready.mjs";
import { readListing } from "./reads.mjs";

export class VerbError extends Error {}

/**
 * Whether a room may be read for people at all, before a page is turned: a
 * parody community, a room whose rules forbid it, an address the platform
 * itself refuses. `{ url }` to read, or `{ refused }` with the sentence. The
 * probe asks it here; the cloud asks it before it hands the operator's browser
 * a read (lib/cloud.mjs), so the two refuse the same rooms for the same words.
 */
export function probeTarget({ p, roomState, place, q = null }) {
  const label = labelsOf(p).room(place);
  if (p.parody?.(place)) return { refused: `${label} is a parody community — the platform's own convention for one. A competitor's picker put one of these top of its list at 100/100.` };
...
/** A post handed in from elsewhere (the cloud's job, the operator's browser):
 *  its id is the platform's, off its own permalink, and its fields are cut to
 *  the lengths the store keeps — so a read made on another machine cannot
 *  write a row a read made here could not. */
function cleanEntry(e) {
  const url = String(e?.url ?? "").split("?")[0].slice(0, 400);
  const id = platformFor(url)?.idOf?.(url) ?? null;
  if (!id || (e?.id && String(e.id) !== id)) return null;
  return {
    kind: "post", id, url,
    author: e.author ? String(e.author).replace(/^u\//, "").slice(0, 60) : null,
    title: String(e.title ?? "").slice(0, 300),
    body: String(e.body ?? "").slice(0, 4000),
    at: e.at ? String(e.at).slice(0, 40) : null,
    comments: Number.isFinite(Number(e.comments)) ? Number(e.comments) : null,
  };
}

/** A whole read handed in as data, checked the same way. */
const cleanRead = (r) => ({
  ok: Boolean(r?.ok),
  error: r?.ok ? null : String(r?.error ?? "the read failed").slice(0, 300),
  redirected: Boolean(r?.redirected),
  finalUrl: r?.finalUrl ? String(r.finalUrl).slice(0, 400) : null,
  subtitle: r?.subtitle ? String(r.subtitle).slice(0, 2000) : null,
  total: Math.max(0, Number(r?.total) || 0),
  entries: (Array.isArray(r?.entries) ? r.entries : []).slice(0, 200).map(cleanEntry).filter(Boolean),
});

/** The verbs, bound to a root, a project directory and a way to speak. */
export function verbs({ root, dir, log = console.log, warn = console.error, browse = null } = {}) {
  let current = "mq";   // the verb running now, for the lane's task label
  browse = browse ?? ((o) => browser(serverBase() ?? "http://127.0.0.1:8787", o));

const ROOT = root;   // the root: the default project, the registry, the machine's files
const DIR = dir;     // the project every verb below acts on
const F = (n) => join(DIR, n);
const now = () => new Date().toISOString();
const die = (m) => { throw new VerbError(m); };

// The registry (skills/ and <ROOT>/skills/) was loaded by the entry point
// before this factory ran. Everything platform-mechanical below comes through
// the active adapter; nothing here names one.
const P = () => preferred(readStash(DIR).platform) ?? die("no platform skill is active — `mq skills` says why");
const L = () => labelsOf(P());
...
// Every read is a page in the operator's own Chrome (lib/browse.mjs). Where
// the lane is comes from the host: the CLI reaches the server that holds it
// (`mq serve` sets MQ_SERVER for the children it spawns; a bare terminal
// reaches the same server when one is running, and otherwise the verb says
// so and stops); the extension's worker hands in its own broker, in-process.
// There is no other way to read, by the operator's rule. Two seats: the
// operator's own session for finding people, the stranger's (an Incognito
// tab) for what became of their own words.
const lane = ({ stranger = false, task = null } = {}) =>
  browse({ task: task ?? `mq ${current}`, stranger, project: DIR });
...
/**
 * Who you are on this platform — asked of the browser first, of you second.
 *
 * The address the platform declares (`account.whoami`) redirects to your own
 * profile while you are signed in, so the handle arrives in the URL bar of a
 * tab you can watch. Nothing on the page is read, nothing is typed into it,
 * and the tab closes after. A platform that declares no such address, or a
 * browser that is signed out, gets the old question instead.
 */
async function whoAmI() {
  const p = P();
  const url = accountOf(p).whoami;
  if (!url) die(`${p.name} cannot say who you are from an address alone — name yourself: mq me <your-username>`);
  const b = lane({ task: "who you are here" });
  try {
    const o = await b.open(url);
    if (o.error) die(o.error);
    const at = await b.where();
    const name = whoamiOf(p, at.url ?? o.url);
    if (!name) die(`that address did not land on a profile, which is what ${p.name} does when the browser is signed out. Sign in there and try again, or name yourself: mq me <your-username>`);
    return name;
  } finally {
    await b.close();
  }
...
cmds.me = async (args) => {
  const asked = String(args[0] ?? "").replace(/^\/?u\//, "").trim();
  const name = asked || (await whoAmI());
...
cmds.sync = async () => {
  ...
  log(`reading ${acct.name}'s profile as a stranger would — an Incognito tab of your own browser\n`);
  const b = lane({ stranger: true, task: `mq sync — ${acct.name} as a stranger` });
  const r = await readPage(b, P().userPage(acct.name));
  await b.close();

  if (!r.ok) {
    if (r.error === "http_404") return log(notFound(acct.name));
    return log(`  could not read it: ${r.error}\n  That is a failed read, not a finding about your account. Try again.`);
  }
  if (r.redirected) return log(`  the profile URL redirected — check the spelling of '${acct.name}'.`);
  ...
  log(`  ${r.entries.length} on the profile, ${fresh} new to the store.`);
  ...
  if (bound) log(`  ${bound} conversation${bound === 1 ? "" : "s"} bound to your own comments — the return pass reads their replies.`);
  if (r.entries.length === 0) {
    log(`\n  Zero entries, logged out, with a 200.`);
    log(`  Either you have not commented, or nothing you wrote is visible to strangers.`);
    log(`  Those look identical from here and the tool will not guess between them.`);
    log(`\n  To tell them apart: paste a permalink you know you posted —`);
    log(`      mq add <permalink>   then   mq check`);
    log(`  If it comes back 'filtered', the profile being empty is not an absence of writing.`);
  } else if (fresh) {
    log(`\n  Next: mq check`);
  }
};
...
cmds.add = (args) => {
  ...
  const { id, kind } = it;
  if (items().has(id)) return log(`already listening to ${id}`);
...
cmds.check = async (args) => {
  ...
  if (!todo.length) return log(`every item has a settled answer. \`mq check --all\` re-reads them — that is how a change gets caught.`);
  ...
  log(`${todo.length} to check across ${groups.size} threads; doing ${work.length}.`);
  // A page turn each, in an Incognito tab of your own browser — and a thread
  // longer than one page costs a second read to settle.
  log(`Each thread is a page turn in an Incognito tab of your browser — the stranger's seat.\n`);
  ...
  for (const [thread, group] of work) {
    ...
    log(`${sub ? L().room(sub) : "?"}  ${group.length} of yours`);
    ...
    for (const it of group) {
      ...
      if (verdict.state === "inconclusive" && it.kind === "comment") {
        log(`    thread is longer than a page — reading the comment's own view`);
      ...
      log(`    ${mark(verdict.state)} ${verdict.state.padEnd(12)} ${line(it)}`);
      if (!verdict.confident) log(`                    ${verdict.why}`);
    }
    ...
    if (!t.ok && /not attached|no server|Incognito/i.test(t.error)) { log(`\n  ${t.error}`); break; }
  }
  await b.close();
  log(`\nmq status`);
};
...
cmds.back = async (args) => {
  ...
  if (!todo.length) return log(`nothing from the last ${days} days that a stranger can still see. \`mq back --days 60\` looks further back.`);
  log(`${todo.length} of your comments from the last ${days} days.`);
  log(`One page each, in an Incognito tab of your browser — only a comment's own view carries its replies.\n`);
  ...
  for (const it of todo) {
    const f = await readPage(b, p.commentPage(it.url));
    if (!f.ok && /not attached|no server|Incognito/i.test(f.error)) { log(`  ${f.error}`); break; }
    ...
    log(`  ${c.state === "waiting" ? "!" : " "} ${c.state.padEnd(9)} ${(c.replies ?? 0)} repl${(c.replies ?? 0) === 1 ? "y" : "ies"}  ${line(it)}`);
  }

  await b.close();
  if (!waiting.length) return log(`\nNobody is waiting on you.`);
  ...
  for (const { it, c } of byUrgency(waiting.map(({ it, c }) => ({ it, c, at: c.at })))) {
    log(`  u/${c.latest.author} · ${ago(c.latest.at)}${c.since_you ? " · since you last spoke" : ""}`);
    log(`    they said: ${(c.latest.text || "").replace(/\s+/g, " ").slice(0, 150)}`);
    log(`    you said:  ${line(it)}`);
    log(`    ${c.latest.url || it.url}\n`);
  }
  log(`You answer these yourself, in your own words. Nothing here writes or sends anything.`);
};
...
async function returnPass(due) {
  ...
  for (const conv of due) {
    ...
    if (!it) continue;
    const f = await readPage(b, P().commentPage(it.url));
    if (!f.ok && /not attached|no server|Incognito/i.test(f.error)) { log(`  ${f.error}`); await b.close(); return -1; }
    const c = conversation(it, f, me);
    append("replies.jsonl", { id: it.id, at: now(), state: c.state, replies: c.replies ?? 0, latest: c.latest ?? null, since_you: c.since_you ?? false });
    const row = noteReturn(it, c);
    read++;
    log(`  ${row?.state === "waiting" ? "!" : " "} ${String(c.state).padEnd(9)} ${row?.author ?? "?"} in ${L().room(conv.place)}`);
  }
  await b.close();
  return read;
}
...
cmds.models = async (args) => {
  ...
  log(`${PLANS[p].title} plan — ${PLANS[p].what}\n`);
  for (const r of Object.values(ROLES)) log(`  ${r.key.padEnd(7)} ${pick[r.key]}`);
  if (p === "local") {
    ...
    if (missing.length) log(`  not pulled yet: ${missing.map((m) => `ollama pull ${m}`).join("  ·  ")}`);
  } else {
    ...
    if (p === "free") log(`  free variants: 20 requests a minute, 50 a day (1,000 once $10 of credit was ever bought) — OpenRouter's limits, read 2026-09-01`);
  }
...
cmds.status = () => {
  ...
  if (!store.size) {
    ...
    if (r && !r.ok && r.err === "http_404") return log(notFound(acct.name, r.at) + backlog);
    return log("nothing stored yet — run `mq sync`." + backlog);
  }
  const tally = new Map();
  const unchecked = [];
  ...
  log(`${acct?.name ?? "?"} — ${store.size} things you said\n`);
  for (const [state, n] of [...tally].sort((a, b) => b[1] - a[1])) {
    log(`  ${String(n).padStart(4)}  ${state.padEnd(13)} ${STATES[state]}`);
  }
  if (unchecked.length) log(`  ${String(unchecked.length).padStart(4)}  ${"unchecked".padEnd(13)} not looked at yet — mq check`);
  ...
  if (moved.length) {
    ...
    for (const { it, h } of moved.slice(0, 10)) {
      log(`  ${h.changed_at.from} -> ${h.changed_at.to}  ${h.changed_at.at.slice(0, 16).replace("T", " ")}`);
      log(`     ${line(it)}\n     ${it.url}`);
    }
  }
...
cmds.log = (args) => {
  ...
  if (!rows.length) return log(id ? `no checks for ${id}` : "no checks yet.");
  const store = items();
  for (const c of rows.slice(-40)) {
    log(`${c.at.slice(0, 16).replace("T", " ")}  ${c.state.padEnd(12)} ${c.id}`);
    log(`   ${c.why}`);
  }
  if (id && store.get(id)) log(`\n${store.get(id).url}`);
};
...
cmds.pull = async (args) => {
  ...
  const token = (args.includes("--token") ? args[args.indexOf("--token") + 1] : null)
    ?? env("MQ_FEED_TOKEN") ?? state[base]?.token;
  ...
  log(`${added} new from ${base}${skipped ? `, ${skipped} already known or already answered` : ""}${pages > 1 ? ` (${pages} pages)` : ""}.`);
...
cmds.sweep = () => {
  ...
  log(`${n} of your bodies and ${m} replies past 48h dropped.`);
  log(`Ids, urls, dates, hashes and every check and reply count are untouched.`);
};
...
cmds.probe = async (args) => {
  ...
  const target = probeTarget({ p: P(), roomState, place, q });
  if (target.refused) return refused(target.refused);
  const { url } = target;

  log(`probing ${label}${q ? ` for "${q}"` : " (new posts)"}${campaign ? ` under the campaign ${campaign}` : ""} — in a tab of your own browser\n`);
  const b = lane({ task: `mq probe ${label}` });
  let r;
  // A person opens the ones that look like them (lib/reads.mjs) — and none
  // at all in a room that does not exist or whose description forbids it.
  try { r = await readListing(b, url, { platform: P(), skip: new Set(found().keys()) }); }
  finally { await b.close(); }
  return recordProbe({ place, q, campaign, url, read: r });
};

/**
 * The record half of a probe: what one read of `url` found, written down —
 * the ledger line, the two refusals only a read can reach (a room that does
 * not exist, a description that forbids it), and the new posts, pending for
 * the judge. The probe's own read ends here, and so does one the cloud asked
 * the operator's browser for (`probed`, Stage 2). Returns how it went.
 */
const recordProbe = ({ place, q = null, campaign = null, url, read: r }) => {
  const label = L().room(place);
  append("reads.jsonl", { url, at: now(), ok: r.ok, err: r.ok ? null : r.error, n: r.ok ? Number(r.total) || 0 : 0, via: "browser" });
  if (!r.ok) { log(`  could not read it: ${r.error}\n  That is a failed read, not a verdict on the room.`); return { outcome: "failed", error: r.error }; }
  // A room that does not exist is answered by a silent redirect, with a 200.
  // Only the final URL gives it away.
  if (r.redirected) { refused(`there is no ${label} — the page redirected to ${r.finalUrl}, which the platform does instead of 404ing.`); return { outcome: "refused", why: "missing" }; }

  // Free, and it catches the blunt cases.
  const desc = fromDescription(r.subtitle);
  if (desc.state === "banned") {
    writeRoom(place, desc.quote);
    refused(`${label}'s own description says: "${desc.quote}"`);
    return { outcome: "refused", why: "banned", quote: desc.quote };
  }

  const known = found();
  const seen = new Set();
  const tag = `${place}:${q ?? "new"}`;
  const fresh = r.entries.filter((e) => e.kind === "post" && !known.has(e.id) && !seen.has(e.id) && seen.add(e.id)).map((e) => ({ id: e.id, place, url: e.url, author: e.author, title: e.title,
  ...
  log(`  ${r.total} posts on the page, ${fresh.length} new to judge${bodies < fresh.length ? ` (${fresh.length - bodies} previewed only — the page showed no body and the ${P().bodiesPerRead}-per-read cap on opening posts was spent)` : ""}.`);
  log(`\n  Its rules are NOT readable from here — measured, and the public description is not the rules list.`);
  log(`  Read them once:  ${rulesUrl(place)}`);
  log(`  Then answer the line in ${roomPath(place)}`);
  log(`\n  Judge what came back:  mq pending    then    mq judge < verdicts.json`);
  return { outcome: "ok", fresh: fresh.length };
};

/**
 * A probe whose read was made elsewhere — the operator's browser, working a
 * job the cloud handed it (Stage 2 of the Quest plan). stdin:
 * { place, q, campaign, url, read } where `read` is lib/reads.mjs readListing's
 * answer. The refusals that need no page run again first: the room may have
 * been answered "no" while the read was out.
 */
cmds.probed = (args, stdin) => {
  let body;
  try { body = JSON.parse(stdin); } catch { die("stdin is not JSON — expected { place, q, campaign, url, read }"); }
  const place = String(body?.place ?? "").replace(/^\/?r\//, "").replace(/[^\w-]/g, "");
  if (!place) die("no place — which room was read?");
  const q = body.q ? String(body.q).slice(0, 120) : null;
  let campaign = body.campaign ? String(body.campaign).toLowerCase().slice(0, 40) : null;
  if (campaign && !readCampaign(DIR, campaign)) campaign = null;
  const target = probeTarget({ p: P(), roomState, place, q });
  if (target.refused) { refused(target.refused); return { outcome: "refused", why: "before" }; }
  return recordProbe({ place, q, campaign, url: target.url, read: cleanRead(body.read) });
};

const refused = (why) => { log(`  refused — ${why}`); log(`  no source written. This is not a score, it is a no.`); };
...
cmds.found = (args, stdin) => {
  ...
  const q = body.q ? String(body.q).slice(0, 120) : null;
  let campaign = body.campaign ? String(body.campaign).toLowerCase().slice(0, 40) : null;
  if (campaign && !readCampaign(DIR, campaign)) { log(`no campaign "${campaign}" in this project — recorded under the general fit instead.`); campaign = null; }
  ...
  log(`${fresh.length} new post${fresh.length === 1 ? "" : "s"} recorded from ${label}${q ? ` for "${q}"` : ""}${campaign ? ` under ${campaign}` : ""}${dupes ? `, ${dupes} already known` : ""}${noId ? `, ${noId} without a post permalink (skipped)` : ""}.`);
  if (fresh.length) log(`They wait on a verdict: mq pending, then mq judge — or the Judge button.`);
  if (room.state === "unanswered") log(`${label}'s rules are not readable from here — a human reads ${rulesUrl(place)} once and answers ${roomPath(place)}.`);
};

cmds.rooms = () => {
  const rows = readJsonl("probes.jsonl");
  const places = [...new Set(rows.map((r) => r.place))];
  if (!places.length) return log(`no rooms probed yet — \`mq probe <room> --q "<phrase>"\``);
  for (const place of places) {
    const st = roomState(place);
    const flag = st.state === "allowed" ? "ok " : st.state === "banned" ? "NO " : "?  ";
    log(`${flag} ${L().room(place).padEnd(26)} ${st.state === "unanswered" ? `rules unread — ${rulesUrl(place)}` : st.state}`);
  }
 ...
 */
cmds.watch = (args) => {
  const place = String(args[0] || die(`usage: mq watch <room> [--q "<phrase>"] [--campaign <id>] [--every <minutes>]`)).replace(/^\/?r\//, "").trim();
  const q = args.includes("--q") ? args[args.indexOf("--q") + 1] : null;
  const campaign = args.includes("--campaign") ? String(args[args.indexOf("--campaign") + 1] ?? "").toLowerCase() : null;
  // How often it is read again. An hour by default; a customer's rooms are
  // read a few times a day (Stage 1 of the Quest plan) — one browser at a
  // person's pace cannot read many rooms every hour.
  const every = args.includes("--every") ? Math.max(30, Math.min(1440, Math.round(Number(args[args.indexOf("--every") + 1]) || 60))) : 60;
  if (campaign && !readCampaign(DIR, campaign)) die(`no campaign "${campaign}" — mq campaigns`);
  ...
  if (sources().some((x) => x.id === id)) return log(`already watching ${id}`);
  append("sources.jsonl", { id, place, q, url, cadence_min: every, added: now(), ...(campaign ? { campaign } : {}) });
  log(`watching ${id}${campaign ? ` under ${campaign}` : ""} — ${decision.why}`);
};

cmds.unwatch = (args) => {
  const id = String(args[0] || die("usage: mq unwatch <source-id>")).toLowerCase();
  append("sources.jsonl", { id, deleted: true, at: now() });
  log(`stopped watching ${id}. What it already found is untouched.`);
};

cmds.sources = () => {
  const all = sources();
  if (!all.length) return log("watching no sources yet.");
  const lr = new Map(readJsonl("reads.jsonl").filter((r) => r.source).map((r) => [r.source, r]));
  const seen = [...found().values()];
  for (const s of all) {
    const r = lr.get(s.id);
    log(`${s.id.padEnd(34)} ${String(seen.filter((f) => String(f.probe ?? "").toLowerCase() === s.id).length).padStart(4)} found   ${!r ? "never read" : r.ok ? `read ${r.at.slice(0, 16).replace("T", " ")}` : `ERROR ${r.err ?? ""}`}${s.campaign ? `   campaign ${s.campaign}` : ""}`);
  }
};
...
cmds.platforms = () => {
  for (const p of platforms()) {
    log(`${p.id.padEnd(12)} ${p.name.padEnd(10)} ${p.origin.padEnd(9)} page turns at least ${Math.round(p.gapMs / 1000)}s apart, in your own browser`);
  }
  log(`\nA platform is a skill: a folder with a SKILL.md and an adapter.mjs.`);
  log(`Built-in ones live in skills/; drop your own into ${DIR}/skills/ and it loads.`);
  log(`The contract is skills/README.md.`);
};
...
cmds.skills = async (args) => {
  if (args[0] === "use") {
    ...
    log(active ? `${slot} → ${active.id}` : `${slot} → nobody (no active skill provides it)`);
    return;
  }
  const st = skillState();
  for (const s of st.active)
    log(`${s.id.padEnd(14)} ${(s.ring === "local" ? "yours" : "built-in").padEnd(9)} ${(s.provides ?? "knowledge").padEnd(22)} ${Object.keys(s.seats).join(", ") || "—"}`);
...
/**
 * The record half of one watched source's read: the ledger line (which is
 * what makes the source not due again for a cadence) and every new post,
 * pending for the judge. `known`, `gone` and `p` are the tick's own, shared
 * across its sources; the caller writes `p` back. Returns { fresh, dark }.
 */
const recordTick = (s, r, { known, gone, p }) => {
  append("reads.jsonl", { url: s.url, source: s.id, at: now(), ok: r.ok, err: r.ok ? null : r.error, n: r.ok ? Number(r.total) || 0 : 0, via: "browser" });
  if (!r.ok) { log(`  ${s.id}: ${r.error}`); return { fresh: 0, dark: /not attached|no server/i.test(r.error ?? "") }; }
  let fresh = 0;
  for (const e of r.entries) {
    if (e.kind !== "post" || known.has(e.id)) continue;
  ...
  log(`  ${s.id}: ${r.total} read, ${fresh} new`);
  return { fresh, dark: false };
};

/**
 * One watched source, read elsewhere — the operator's browser, on a job the
 * cloud handed it (Stage 2). stdin: { source, read } with `source` the id
 * `watch` gave it. A source unwatched while the read was out records nothing.
 */
cmds.ticked = (args, stdin) => {
  let body;
  try { body = JSON.parse(stdin); } catch { die("stdin is not JSON — expected { source, read }"); }
  const s = sources().find((x) => x.id === String(body?.source ?? "").toLowerCase());
  if (!s) { log(`no watched source ${body?.source ?? "(none)"} — nothing recorded`); return { fresh: 0, gone: true }; }
  const p = pending();
  const got = recordTick(s, cleanRead(body.read), { known: found(), gone: contacted(), p });
  setPending(p);
  return got;
};
...
cmds.tick = async (args) => {
  ...
  if (!due.length && !dueC.length) { noteDigest(); return log("nothing is due."); }
  if (due.length) log(`${due.length} source${due.length === 1 ? "" : "s"} due — read in a tab of your own browser, a page turn every few seconds.\n`);
  ...
  for (const s of due) {
    const r = await readListing(b, s.url, { platform: P(), skip: new Set(known.keys()), skipAuthors: gone, checkDescription: false });
    const got = recordTick(s, r, { known, gone, p });
    if (got.dark) { dark = true; break; }
    total += got.fresh;
  }
  if (b) await b.close();
  setPending(p);
  ...
  if (dueC.length && !dark) {
    ...
    if (back < 0) log(`  the stranger's seat is not open — Chrome needs "Allow in Incognito" for the extension; the conversations stay due.`);
  }
  const w = waitingRows(S).length;
  if (w) log(`\n${w} waiting on you — mq waiting`);
  const ub = unbound(S).length;
  if (ub) log(`${ub} conversation${ub === 1 ? "" : "s"} not yet found on your profile — mq sync binds them.`);
...
cmds.waiting = (args) => {
  ...
  if (args.includes("--json")) return log(JSON.stringify(rows, null, 2));
  const all = [...conversationRows(S).values()].filter((c) => c.state !== "closed");
  if (!rows.length) {
    log(`nobody is waiting on you. ${all.length} conversation${all.length === 1 ? "" : "s"} tracked${unbound(S).length ? `, ${unbound(S).length} not yet found on your profile (mq sync)` : ""}.`);
    // Named, so whoever reads this — the specialist included — can say who
    // is tracked without guessing from the deck.
    for (const c of all) log(`  ${String(c.state).padEnd(9)} u/${c.author ?? "?"} in ${L().room(c.place)} · opened ${ago(c.opened_at)}${c.campaign ? `  [${c.campaign}]` : ""}${c.state === "sent" ? " — not yet found on your profile" : ""}`);
    return;
  }
  log(`${rows.length} waiting for you — oldest first:\n`);
  for (const r of rows) {
    log(`  ${r.id}  u/${r.author} in ${L().room(r.place)} · ${ago(r.at)}${r.campaign ? `  [${r.campaign}]` : ""}`);
    log(`    they said: ${r.they_said.replace(/\s+/g, " ").slice(0, 150)}`);
    log(`    you said:  ${r.you_said.replace(/\s+/g, " ").slice(0, 150)}`);
    log(`    ${r.url}\n`);
  }
  log(`mq draft <id> writes the next turn's material; you post it yourself.`);
};

cmds.pending = () => {
  const p = pending(), all = found();
  if (!p.length) return log("nothing waiting on a verdict.");
  log(JSON.stringify(p.map((x) => {
...
cmds.judge = (args, stdin) => {
  ...
  for (const v of vs) {
    ...
    if (!id) { warn(`  no item numbered ${v.n} — skipped`); continue; }
  ...
  setPending(missing);
  log(`judged ${seen.size} · ${fits} queued · rubric ${rule}`);
  // Unjudged is its own state, and it is loud. It is not a no.
  if (missing.length) log(`${missing.length} came back with NO VERDICT and stay pending — run judge again`);
  ...
  for (const probe of new Set(p.map((x) => x.probe).filter(Boolean))) {
    ...
    log(`  ${probe}: ${d.why}`);
    if (d.commit) log(`    commit it with:  mq watch ${probe.split(":")[0]}${probe.endsWith(":new") ? "" : ` --q "${probe.split(":").slice(1).join(":")}"`}`);
  }
};
...
cmds.queue = (args) => {
  ...
  if (args.includes("--json")) return log(JSON.stringify(rows, null, 2));
  if (!rows.length) return log("queue is empty.");
  for (const r of rows) {
    log(`\n${r.id}  ${L().room(r.place)}  ${(r.posted_at ?? r.seen_at).slice(0, 16).replace("T", " ")}  ${r.author ?? "?"}${r.campaign ? `  [${r.campaign}]` : ""}`);
    log(`  ${(r.title || "").slice(0, 90)}`);
    log(`  ${(r.body || "").replace(/\s+/g, " ").slice(0, 160)}`);
    log(`  ${r.url}`);
  }
...
cmds.mark = (args) => {
  ...
  if (mark === "sent" && !args.includes("--anyway")) {
    ...
    if (stop) {
      log(`  not logged — ${stop}.\n`);
      log(`  The shape that cost this project its visibility was eleven replies in`);
      log(`  83.9 minutes across seven subreddits with no history in any of them.`);
      log(`  The limits here are ${PER_ROOM_24H} a room and ${OVERALL_24H} overall in 24 hours.\n`);
      log(`  If you have already posted it, say so:  mq mark ${id} sent --anyway`);
      return;
    }
  }
  ...
  if (mark === "sent" && it.author) {
    ...
    log(`logged as answered: ${id}\n  ${it.author} will never appear in a queue again — in any project.`);
  } else log(mark === "sent" ? `logged as answered: ${id}` : `discarded: ${id}`);
};
...
cmds.voice = () => {
  ...
  log(`${bodies.length} of your comments were long enough to measure${all > bodies.length ? ` (${all - bodies.length} were under ${MIN_SAMPLE_CHARS} characters)` : ""}.\n`);
  const rules = voiceRules(merged);
  const summary = voiceSummary(merged);
  log(`${summary ?? "Nothing about your style is measurable yet, and nothing is being guessed."}\n`);
  ...
  if (!summary) {
    ...
    log(`stays silent until your own comments show it — run \`mq sync\` for more samples.`);
  }
...
cmds.draft = (args, stdin) => {
  ...
  log(turn > 1 ? `# Answer this person — turn ${turn}\n` : `# Answer this person\n`);
  log(`${it.author ?? "?"} in ${L().room(it.place)} — ${it.url}${c ? `\nCampaign: ${c.name} (${c.id}, mention: ${c.mention})` : ""}\n`);
  log(`## What they said\n\n${it.title ? `**${it.title}**\n\n` : ""}${(it.body || "").slice(0, 1600)}\n`);
  if (turn > 1) {
    log(`## The exchange so far\n`);
    for (const t of conv.turns ?? []) log(`--- ${t.by === "you" ? "you" : `u/${t.author ?? "them"}`} · ${String(t.at ?? "").slice(0, 16).replace("T", " ")} ---\n${String(t.text ?? "").slice(0, 1600)}\n`);
    log(`## What they wrote back — answer THIS\n\n${String(conv.latest?.text ?? (conv.turns ?? []).filter((t) => t.by === "them").pop()?.text ?? "").slice(0, 1600)}\n`);
  }
  log(`## How to write it\n`);
  log(signalWritingRules({
    intent: "leads", stage,
...
const saveDraft = (it, text, fp, { turn = 1 } = {}) => {
  ...
  log(`${checked.length === 1 ? "draft" : `${checked.length} drafts`} saved for ${it.id}${turn > 1 ? ` (turn ${turn})` : ""}${round > 1 ? ` — round ${round}` : ""}\n`);
  ...
  for (const d of checked) {
    ...
    clean = false;
    if (checked.length > 1) log(`[${styleLabel(d.style)}]`);
    if (took.length) {
      log(`!! THEIRS, NOT YOURS — the draft says you built ${took.map((n) => `"${n}"`).join(", ")}, which is the thing THEY posted about:`);
      log(`   Delete the claim. The person you are replying to can see it is false at a glance; me.md names what is yours.\n`);
    }
    if (rep) {
      log(`!! REPEATED PHRASING — ${rep.length} identical consecutive words you have used before:`);
      log(`   "${rep.phrase}"`);
      log(`   Reddit names "the same or similar comments across communities" as reportable spam — and a campaign is a direction, never a template.`);
      log(`   The corpus this was measured on shared a 26-word run while its duplicate check reported clean.\n`);
    }
    if (links.length) {
      log(`!! INVENTED LINK — not present in the thread we read:`);
      for (const u of links) log(`   ${u}`);
      log(`   Delete it. It is a checkable false statement under your own name.\n`);
    }
    if (said.length) {
      log(`?? CLAIMS ABOUT YOU — each is either true or it is the thing that ends the account:`);
      for (const c of said) log(`   "${c.sentence}"`);
      log(`   Check each against ${DIR}/me.md. A competitor posted "at my last job i used <product>"`);
      log(`   into a clinical thread under a real name. Nobody had ever had that job.\n`);
    }
    if (tell.length) {
      log(`?? READS LIKE A TEMPLATE — ${tell.length} phrase${tell.length === 1 ? "" : "s"} nobody types to one person: ${tell.map((t) => `"${t}"`).join(", ")}`);
      log(`   Rewrite those sentences in your own words. They are what makes a comment read as generated.\n`);
    }
  }
  if (clean) log(`No repeated phrasing, no invented links, no claims about your history, no template phrases.`);
  log(`You send it yourself, from your own account. Nothing here posts.`);
};
...
cmds.ready = (args) => {
  ...
  if (!names.length) return log(only ? `nothing known about ${L().room(only)} yet.` : `no rooms yet — \`mq sync\` to read your own history, or \`mq probe <room>\`.`);
  ...
  for (const place of names) {
    ...
    log(`  ${label}  ${L().room(place)}`);
    log(`             ${wrap(v.why, 66, "             ")}`);
    if (r?.quiet_days != null && r.quiet_days > 14 && v.state !== "not ready")
      log(`             last comment ${r.quiet_days} days ago`);
  }
...
cmds.campaigns = (args) => {
  ...
  if (args.includes("--json")) return log(JSON.stringify(all, null, 2));
  if (!all.length) return log(`no campaigns yet. Describe a tactic to the specialist on the panel and it proposes one as cards — or write ${join(DIR, "campaigns")}/<id>.md by hand (the shape is in lib/campaigns.mjs).`);
  for (const c of all) log(`${c.status === "active" ? "on " : "off"} ${c.id.padEnd(28)} ${c.name.slice(0, 40).padEnd(40)} mention: ${c.mention.padEnd(9)} rubric ${c.hash}`);
...
cmds.projects = (args) => {
  ...
  if (args.includes("--json")) return log(JSON.stringify(all, null, 2));
  for (const p of all) log(`${p.current ? "* " : "  "}${p.id.padEnd(24)} ${p.name.slice(0, 30).padEnd(30)} ${p.dir}`);
...
cmds.serve = async (args) => {
  // Not a literal on purpose: a bundler follows literal imports, and the quest
  // Edge Function (Stage 2) bundles this file — it must not carry the
  // dashboard, and everything the dashboard imports, along with it.
  const at = "../bin/serve.mjs";
  const { serve } = await import(at);
  ...
  const api = {};
  for (const [name, fn] of Object.entries(cmds)) api[name] = (args = [], stdin = "") => { current = name; return fn(args, stdin); };
  return api;
}
