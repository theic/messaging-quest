...
export function openConversation(S, it, { text = "", at = new Date().toISOString(), via = "panel" } = {}) {
  const row = {
    ...
    turns: [{ by: "you", text: String(text ?? "").slice(0, 4000), at, via }],
  };
...
export function recordTurn(S, conv, { text = "", at = new Date().toISOString(), via = "panel" } = {}) {
  const row = { ...conv, turns: [...(conv.turns ?? []), { by: "you", text: String(text ?? "").slice(0, 4000), at, via }], state: "answered", answered_at: at };
  S.append("conversations.jsonl", row);
  return row;
}
