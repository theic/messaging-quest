// The memory — four markdown files, and the rule that nothing writes them
...
//
// These four are also the agent's filesystem. `lib/agents.mjs` mounts this
...
// four names and no others: an editor that can write anywhere is a shell, and
...
export const RULE_SEED = `# Fit rule
...
export const EDITABLE = MEMORY.map((m) => m.file);

const meta = (file) => MEMORY.find((m) => m.file === file) ?? null;

/** Reject anything that is not one of the four, by exact basename. `join` on an
...
export function writeMemory(dir, file, body) {
  if (!allowed(file)) throw new Error(`${file} is not one of the four memory files`);
 ...
 *  server on boot, so an older `.earshot/` grows the new files on first run. */
...
export function memoryContext(dir) {
  return readMemory(dir)
    .filter((m) => m.filled && m.file !== "rule.md")
    .map((m) => `# ${m.title} (${m.file})\n\n${m.body.trim()}`)
    .join("\n\n---\n\n");
}

/** How much of the setup is actually answered — the number the /setup progress
 *  bar and the "you are not set up yet" banners both read. */
export function memoryProgress(dir) {
  const all = readMemory(dir);
  return { done: all.filter((m) => m.filled).length, total: all.length, files: all };
}
