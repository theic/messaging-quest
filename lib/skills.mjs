...
import { existsSync, readdirSync, readFileSync, writeFileSync, join, builtinDir } from "./fs.mjs";

/** The built-in ring: the repo's skills/ under Node, "/skills" on the
 *  extension's memory host (lib/fs.mjs says which). */
...
// Nothing loads at import (0.10.0). The registry being empty is a QUIET
// failure — rooms stop resolving and nothing errors — which is why every
// entry point calls loadPlatforms(dir) (and so loadSkills) before its first
// question. Importing used to be enough for the built-ins; the extension's
// worker installs its filesystem AFTER its modules evaluate, and a scan at
// import time there found no filesystem at all.
