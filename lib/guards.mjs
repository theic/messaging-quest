...
/**
 * "I got seven booked calls in one day doing that manually" — a free writer,
 * 2026-09-18, drafting for a customer whose me.md said only what they built.
 * A RESULT of yours — I or we, a verb that reports an outcome, and a number —
 * is the sentence a reader believes most and the one nothing here can know.
 * `claims` above looks for the verbs of experience and misses it; this looks
 * for the outcome. Same contract: found and shown, never rejected.
 */
const RESULT = /\b(?:i|we)\b[^.!?\n]{0,40}\b(?:got|gets|made|landed|booked|closed|signed|grew|doubled|tripled|hit|reached|earned|sold|generated|saw|had|went from)\b/i;
const NUMBER = /\b(?:\d[\d,.]*\s*[%kx]?|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|fifteen|twenty|thirty|forty|fifty|hundred|thousand|dozen)\b/i;
export function results(text) {
  const out = [];
  for (const s of String(text ?? "").split(/(?<=[.!?])\s+|\n+/)) {
    const t = s.trim();
    if (t && RESULT.test(t) && NUMBER.test(t)) out.push(t.slice(0, 240));
  }
  return out;
}
