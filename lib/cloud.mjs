...
import { campaignOf, secondPass, lookNote, list, isFresh, offerFrom, saysNothing, discoveredPlaces, placesFrom, ackFor, uniqCI, EVERY_MIN, CAMPAIGN } from "./quest.mjs";
...
/** Before the offer is shown, the platform's own search for communities is
 *  read for the offer's topics — this many, this long at most. A read that
 *  never comes back (a wall nobody lifts) must not hold the card for ever:
 *  past it, the offer goes out with the model's own guesses. */
const DISCOVER_TOPICS = 3;
const DISCOVER_WAIT_MS = 15 * 60_000;
...
export function cloud({ db, skills = [], adapters = {}, env = {}, quest = null, models = AGENTS, log = () => {}, now = () => Date.now() } = {}) {
  ...
  async function run(id, { budgetMs = 110_000 } = {}) {
    ...
    try {
      ...
      const clock = async () => {
        ...
        await discoverStep({ dir, byId, consume, room, P });
      ...
      const models_ = hasModel(dir) ? [
        ...
        ["offer", () => offerModelStep({ dir, byId, consume, ask, P })],
      ...
      const held = new Set([readStash(dir).site_job, ...(readStash(dir).discovering?.jobs ?? []), ...(readStash(dir).look?.rooms ?? []).flatMap((r) => [r.rulesJob, r.probeJob])].filter(Boolean));
  ...
  async function offerStep({ dir, byId, consume, inFlight, ask }) {
    ...
    if (st.offer || st.scout_empty || st.discovering) return;
  ...
  async function offerModelStep({ dir, byId, consume, ask, P }) {
    const st = readStash(dir);
    if (st.offer || st.scout_empty || st.discovering || modelWait(st.cloud_offer_fail)) return false;
    const c = customerOf(dir) ?? {};
    const job = st.site_job ? byId.get(st.site_job) : null;
    ...
    try {
      ...
      if (proposal && (proposal.one_line || proposal.rule_md)) {
        // The communities a model names are guesses (five names in five
        // were wrong on the first live run, 2026-09-19): where the problem is
        // said is read from the platform itself first, and the card goes out
        // with rooms that exist (discoverStep).
        const asking = await askDiscovery(dir, proposal, { ask, P });
        if (asking) patchStash(dir, { discovering: asking });
        else postOffer(dir, proposal, { room: roomOf(dir) });
      }
  ...
  /** The platform's own search for communities, asked of the browser once a
   *  topic: what a person does to find where to look. Null when there is
   *  nothing to ask (a platform with no such page, an offer that says
   *  nothing, no topic and no phrase to stand in for one) — the offer then
   *  goes out as it is. */
  async function askDiscovery(dir, proposal, { ask, P }) {
    const p = P();
    const offer = offerFrom(proposal);
    const topics = uniqCI([...offer.topics, ...offer.searches]).slice(0, DISCOVER_TOPICS);
    if (typeof p?.discoverUrl !== "function" || typeof p?.discoverRooms !== "function" || saysNothing(offer) || !topics.length) return null;
    const jobs = [];
    for (const q of topics) {
      const j = await ask("text", { purpose: "discover", q, url: p.discoverUrl({ q }) }, "Finding where people talk about it");
      jobs.push(j.id);
    }
    return { proposal, jobs, at: new Date(now()).toISOString() };
  }

  /** The offer, once the discovery reads are back (or given up on): the
   *  communities the platform lists for the topics first — alive, real,
   *  best of each topic in turn — and the model's own guesses only to fill
   *  the list. No model: the pages are read for the names on them. */
  async function discoverStep({ dir, byId, consume, room, P }) {
    const st = readStash(dir);
    const d = st.discovering;
    if (!d) return;
    if (st.offer) { patchStash(dir, { discovering: null }); return; }
    const js = (d.jobs ?? []).map((id) => byId.get(id)).filter(Boolean);
    const waited = now() - Date.parse(d.at ?? "");
    if (js.some(reading) && !(Number.isFinite(waited) && waited > DISCOVER_WAIT_MS)) return;
    const p = P();
    const lists = js.filter((j) => finished(j) && !failure(j)).map((j) => p?.discoverRooms?.(j.result?.text ?? "") ?? []);
    const found = discoveredPlaces(lists, { parody: p?.parody });
    const places = placesFrom(found, d.proposal?.places ?? []);
    log(found.length
      ? `communities for its topics: ${found.slice(0, 6).map((f) => `${f.place}${f.visitors != null ? ` (${f.visitors}/wk)` : ""}`).join(", ")} — the offer names ${places.join(", ")}`
      : `no communities turned up for its topics${js.length ? "" : " (the reads are gone)"} — the offer keeps the model's own guesses`);
    postOffer(dir, { ...d.proposal, places }, { room });
    patchStash(dir, { discovering: null });
    for (const j of js) consume(j);
  }
  ...
  async function turnStep({ dir, lane, room }) {
    ...
    try {
      ...
      if (!applied?.done) {
        ...
        const history = quest.history ? quest.history(dir, { before: t.row }) : [];
        const ask = (extra = "") => quest.turn(dir, t.text, { history, note: (applied?.note ?? "") + extra, lane, signal });
        let reply = await ask();
        // Somebody wrote to Quest and is waiting. "noted" is the heartbeat's
        // word for having nothing to add; a free model answered a first
        // message with it three times of three (measured 2026-09-19, with a
        // browser attached), and dropping it left a person who had just typed
        // their address in a silent chat. Ask once more, plainly, then say
        // the one thing that is always true. (When an instruction was applied
        // and told to them, "noted" is still the whole answer.)
        if (!applied && saidNothing(reply)) {
          log(`Quest had nothing to say to a message (${JSON.stringify(String(reply ?? "").slice(0, 30))}) — asking once more`);
          reply = await ask("They wrote to you and are waiting for an answer: reply to them in a sentence or two — what you are doing about what they sent. Not the word noted.\n\n");
          if (saidNothing(reply)) {
            const s = readStash(dir);
            const url = customerOf(dir)?.url ?? null;
            let host = null;
            try { host = url ? new URL(url).host.replace(/^www\./, "") : null; } catch { host = null; }
            reply = ackFor({ host, offer: s.offer ?? null, reading: !s.offer && Boolean(s.site_job || s.discovering) });
          }
        }
        if (reply && !saidNothing(reply)) say(dir, reply);
        else if (reply) log(`Quest said ${JSON.stringify(String(reply).slice(0, 30))} to an instruction it had already applied`);
      }
  ...
  function summaryOf(dir, { jobs }) {
    ...
    return {
      ...
      offer: st.offer?.state ?? (st.scout_empty ? "empty" : st.scout_gaveup ? "gave up" : st.site_job || st.discovering ? "reading" : null),
