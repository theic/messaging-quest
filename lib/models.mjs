...
import { existsSync, readFileSync, writeFileSync, unlinkSync, join, env } from "./fs.mjs";
 ...
 *                                   RETIRED by OpenRouter on 2026-09-07 (404 "unavailable for free")
...
export const FREE_MODELS = {
  ...
  "nvidia/nemotron-3-super-120b-a12b:free": {
    ...
    note: "16.9s, 1,044 reasoning tokens, conformed, agreed (2026-09-01). As the scout: three pages and a conforming proposal in 102s + 18s — and 'temporarily overloaded' on three of five asks that afternoon. The free scout and writer since 2026-09-07, when MiniMax M2.7's free variant went paid.",
  },
  // minimax/minimax-m2.7:free was here — the free scout and writer, the best
  // free run measured (three pages in 27s, three specific options in 13s).
  // On 2026-09-07 OpenRouter answered it with 404 "This model is unavailable
  // for free. The paid version is available now", and a model-level
  // fallback does not cover an id that 404s: every draft failed at once, and
  // silently until the strip. Gone from this menu, like glm-5.2 below.
  ...
  // z-ai/glm-5.2:free was here (429 from Decart on both asks, 2026-09-01).
  // Gone from OpenRouter's own /api/v1/models on 2026-09-06, so it is gone
  // from this menu too: a pick that 404s an hour into a tick is not a pick.
...
// FREE IS THE DEFAULT (0.8.0). The tool is meant to be usable for nothing:
// a free OpenRouter key (no card) and the free variants below fill every
// seat, and the paid plan is a choice somebody makes on Settings, not a
// bill they discover. The order here is the order the screens show.
export const PLANS = {
  free: {
    key: "free", title: "Free", needsKey: true, menu: FREE_MODELS,
    what: "OpenRouter's free models. A free key — no card, no bill — and nothing is ever charged. The platform's limits: 20 requests a minute and 50 a day on a fresh account (1,000 a day once $10 of credit was ever bought, which is not required). Slower than paid, and a provider can be full for a minute.",
  },
  paid: {
    key: "paid", title: "Paid", needsKey: true, menu: MODELS,
    what: "OpenRouter's measured defaults on the same key, with credit on it. Cents per hundred verdicts; the writer is the seat that costs.",
  },
  local: {
...
/** Where a free key comes from. On the card, on Settings, in the README. */
...
export const ROLES = {
  ...
  scout: {
    ...
    pick: {
      ...
      // MiniMax M2.7's free variant was the pick until OpenRouter retired it
      // (404, 2026-09-07). Nemotron 3 Super is the next measured free run;
      // the fallbacks are other providers, for the day NVIDIA is full.
      free: { def: "nvidia/nemotron-3-super-120b-a12b:free", alternates: ["inclusionai/ling-3.0-flash-fin:free", "poolside/laguna-s-2.1:free"] },
      local: { def: "qwen3.5:9b", alternates: [] },
    },
    maxTokens: 16384,
  },
  writer: {
    ...
    pick: {
      ...
      // Same story as the scout's: MiniMax M2.7's free variant went paid on
      // 2026-09-07. Nemotron 3 Super writes here until a measurement says
      // which free model writes better; Laguna is measured generic as a writer
      // and is the last resort, not the pick.
      free: { def: "nvidia/nemotron-3-super-120b-a12b:free", alternates: ["inclusionai/ling-3.0-flash-fin:free", "poolside/laguna-s-2.1:free"] },
      local: { def: "qwen3.5:9b", alternates: [] },
    },
    maxTokens: 8192,
...
const defaultFor = (role, p) => ROLES[role].pick[p].def;
export const alternatesFor = (role, p) => ROLES[role].pick[p].alternates;

/* ------------------------------------------------------------------ config */

// The seats and the key are the MACHINE's, not a project's: a child project
// (lib/dirs.mjs) reads and writes them at the root.
const CONF = (dir) => join(sharedDir(dir), "models.json");
const KEYFILE = (dir) => join(sharedDir(dir), "openrouter.key");
...
function conf(dir) {
  ...
  return {
    plan: PLAN_IDS.includes(raw.plan) ? raw.plan : DEFAULT_PLAN,
    picks,
    local: {
...
export function localConfig(dir) {
  ...
  return {
    baseUrl: (env("MQ_LOCAL_URL") || c.baseUrl).replace(/\/+$/, ""),
    key: env("MQ_LOCAL_KEY") ?? c.key,
  };
}
...
export const keySource = (dir) =>
  env("OPENROUTER_API_KEY") ? "env" : existsSync(KEYFILE(dir)) ? "file" : null;
...
export function seat(dir, role) {
  ...
  if (!key) throw new Error(`no OpenRouter key — make a free one at ${KEY_URL} (no card) and paste it on the panel's Settings tab or /settings, or set OPENROUTER_API_KEY; or pick Local and run a model on this machine`);
  return {
    plan: p, key,
    baseUrl: BASE_URL,
    model,
    ...
    // A host with a shorter life says so (MQ_MODEL_TIMEOUT_MS): the quest
    // Edge Function is ended at 150 seconds, and a call that outlives the
    // function it runs in has no one to answer.
    timeoutMs: Math.min(p === "free" ? 180_000 : 120_000, Number(env("MQ_MODEL_TIMEOUT_MS")) || Infinity),
