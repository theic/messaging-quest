...
import { existsSync, readFileSync, writeFileSync, join, sha256, randomHex } from "./fs.mjs";
 ...
 *
 * Compared in constant time over the hashes. A plain `===` on a secret
...
export function validToken(dir, presented) {
  const want = hash(String(presented ?? ""));
  for (const t of tokens(dir)) if (typeof t.sha === "string" && same(t.sha, want)) return t;
  return null;
}
