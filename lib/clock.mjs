 ...
 * `who` is the person on the other end: the operator, or — for Quest, the
 * customer-facing seat — the customer.
 */
export function clockText({
  now = Date.now(), timeZone = zone(),
  operatorAt = null, heartbeatAt = null, tickAt = null, wokeBy = null, who = "the operator",
} = {}) {
  const head = `The clock: ${localTime(now, timeZone)} in ${timeZone} — ${new Date(now).toISOString()}.`;
  const parts = [];
  if (operatorAt) parts.push(`${who} last said something ${ago(operatorAt, now)}`);
