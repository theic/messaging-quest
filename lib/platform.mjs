 ...
 *  words that open one and the words on a reply box. The words that may
 *  never be pressed are the extension's, not a platform's. */
export const composerOf = (url) => {
  const p = platformFor(url) ?? first();
  const c = p?.composer ?? {};
  return { opens: Array.isArray(c.opens) ? c.opens : [], replies: Array.isArray(c.replies) ? c.replies : [], hosts: Array.isArray(c.hosts) ? c.hosts : [] };
};
