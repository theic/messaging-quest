...
export const NO_SERVER = "no server to reach the browser through — run `mq serve` (the panel and the reads both live there) and try again.";
export const NO_STRANGER = "the stranger's view needs an Incognito tab, and Chrome has not allowed the extension there yet: chrome://extensions → Messaging Quest → Details → “Allow in Incognito”. A signed-in read cannot see a shadow-removal, so this is refused rather than answered from the wrong seat.";
