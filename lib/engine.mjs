...
import { offerFrom, offerCard, filesOf, campaignOf, meOf, lookRooms, opportunityText, EVERY_MIN, CAMPAIGN, FIRST_LOOK } from "./quest.mjs";
...
export function engine({ root, base = null } = {}) {
  ...
  function quest(dir, text) {
    ...
    (async () => {
      try {
        ...
        const { reply } = await strategist(dir, text, "chat");
  ...
  function signup(body = {}) {
    ...
    if (known) pj = useProject(DATA, known.id);
      ...
      const base = slug(url ? siteHost(url) : email.split("@")[0]) || "customer";
      let id = base;
  ...
  function autoOffer() {
    try {
      ...
      const tries = Number(st.scout_tries) || 0;
      ...
      if (job && Date.now() - Date.parse(job.finishedAt ?? job.startedAt) < OFFER_RETRY_MS) return;
      if (material) {
        const started = J.run("scout", async (ctl) => {
  ...
  function readRules(dir, place) {
    ...
    return J.run("rules", async (ctl) => {
      const b = browser(LANE, { task: `reading ${label}'s rules`, project: dir });
      try {
        const o = await b.open(LB().rulesUrl(place));
        if (o.error) throw new Error(o.error);
        const t = await b.text(20_000);
        if (t.error) throw new Error(t.error);
        const hit = bansPromotion(t.text);
  ...
  function autoLook() {
    ...
    for (const r of rooms) {
      if (r.rules === "reading") {
        ...
        else if (j.status === "ok") set(r, { rules: J.result(r.rulesJob)?.state === "banned" ? "banned" : "allowed", rulesJob: null });
    ...
    if (settled && !st.look.said && !S.pending().length && !J.busy("judge")) {
      ...
      const verdicts = S.verdicts();
      const mine = [...S.found().values()].filter((f) => rooms.some((r) => f.probe === `${r.place}:${r.q ?? "new"}`));
      const read = mine.length;
      const watched = rooms.filter((r) => r.watch === "watched").map((r) => LB().room(r.place));
      if (!mine.some((f) => verdicts.get(f.id)?.fit)) {
        say(dir, watched.length
          ? `I read ${read} recent post${read === 1 ? "" : "s"} and nobody is asking for this yet. I'm watching ${watched.join(", ")} and will come back here the moment someone does.`
          : `I read ${read} recent post${read === 1 ? "" : "s"} in ${rooms.map((r) => LB().room(r.place)).join(", ")} and found nobody asking for this — and none of them is worth watching for it. Where else do your buyers talk about the problem? Name a community, or tell me more about who buys.`);
      }
      st.look.said = true;
      changed = true;
    }
    if (changed) patchStash(dir, { look: { ...st.look, rooms } });
  }
  ...
  const excerpt = (s, n) => { const t = String(s ?? "").replace(/\s+/g, " ").trim(); return t.length > n ? t.slice(0, n - 1) + "…" : t; };
  ...
  function autoDeliver() {
    ...
    if (today.length < OVERALL_24H) {
      const next = queueRows().find((r) => !had.has(r.id) && S.roomState(r.place).state !== "banned"
    ...
    if (!want) return;
    const round = S.drafts().filter((d) => d.id === want.card.item).pop();
    if (round) {
      ...
      if (drafts.length) setCard(dir, want.id, { draft: drafts[0].text, drafts: drafts.map((d) => ({ style: d.style, text: d.text })) });
      return;
    }
    if (J.busy("draft") || !hasModel(dir)) return;
    const failed = lastOf(dir).draft;
    if (failed?.id === want.card.item) {
      if (/declined/.test(failed.error ?? "")) { setCard(dir, want.id, { no_draft: failed.error }); return; }
      if (Date.now() - failed.at < 10 * 60_000) return;
    }
    startAgentic("draft", [want.card.item]);
  }
  ...
  function confirmOffer(dir) {
    ...
    patchStash(dir, {
      ...
      look: { rooms: lookRooms(offer.places.slice(0, FIRST_LOOK), offer.problem || null), at: new Date().toISOString() },
    });
    setCard(dir, offer.card, { state: "confirmed" });
    const where = offer.places.slice(0, FIRST_LOOK).map((p) => LB().room(p));
    say(dir, `Good. I'll start with ${where.join(", ") || "the communities where this comes up"} and bring you the people I find, here, as I find them. The first look takes some minutes.`);
  ...
  function chatAct(body = {}) {
    ...
    if (m.card.kind === "opportunity") return actOpportunity(dir, m, String(body.action ?? ""));
  ...
  async function actOpportunity(dir, m, action) {
  ...
  return {
    ...
    signup, chatView, chatSend, chatAct, quest, autoOffer, postOffer, confirmOffer, autoLook, autoDeliver, customerClock,
