...
export function engine({ root, base = null } = {}) {
  ...
  function autoTick() {
    try {
      ...
      if (["probe", "sync", "back", "check", "watch", "pull", "add", "scout", "me"].some((v) => J.busy(v))) return;
      const due = dueNow();
  ...
  const stop = () => { clearInterval(sweeper); clearInterval(judgeClock); clearInterval(tickClock); };
