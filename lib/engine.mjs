...
import { rulesVerdict, roomRead } from "./rules.mjs";
import { readText } from "./reads.mjs";
import { queueOf, postOffer as postOfferOf, confirmOffer as confirmOfferOf, chatAct as chatActOf, applyInstruction as applyInstructionOf, deliverStep, heardFrom } from "./customer.mjs";
...
export function engine({ root, base = null, home = null } = {}) {
  ...
  function dueNow(sources = null) {
    return {
      // The tick's own rule, asked of the store (lib/store.mjs due).
      sources: S.due(Date.now(), sources).length,
  ...
  async function applyInstruction(dir, text, interpret = null) {
    if (!readStash(dir).offer) return null;
    const lazy = async (d, t) => (await import("./agents.mjs")).interpretInstruction(d, t);
    return applyInstructionOf(dir, text, { interpret: interpret ?? lazy, room: (p) => LB().room(p) });
  }
  ...
  function readRules(dir, place) {
    ...
    return J.run("rules", async (ctl) => {
      ...
      try {
        const t = await readText(b, LB().rulesUrl(place), { max: 20_000 });
        if (!t.ok) throw new Error(t.error);
        const v = rulesVerdict(t.text);
        ...
        S.writeRoom(place, roomRead(base, v, new Date().toISOString().slice(0, 10)));
        ctl.log(v.quote ? `${label} forbids it: "${v.quote}"` : `${label}'s rules allow it`);
        return v;
      } finally {
        await b.close();
      }
  ...
  // The delivery itself is lib/customer.mjs deliverStep — the cloud runs the
  // same one; here the writer is a job of this process's.
  function autoDeliver() {
    const dir = P();
    const d = deliverStep(dir, { S, room: (p) => LB().room(p), tell: (e) => INBOX(e, dir), lastDraft: lastOf(dir).draft, busy: J.busy("draft") || !hasModel(dir) });
    if (d.draft) startAgentic("draft", d.draft.args);
  }
  ...
  // The offer card, "Looks right" and the buttons on a card are
  // lib/customer.mjs's; the cloud runs the same ones.
  const room = (p) => LB().room(p);
  const postOffer = (dir, proposal) => postOfferOf(dir, proposal, { room, tell: (e) => INBOX(e, dir) });
  const confirmOffer = (dir) => confirmOfferOf(dir, { room, tell: (e) => INBOX(e, dir) });
  const chatAct = (body = {}) => { const dir = P(); return chatActOf(dir, body, { room, mark: (args) => runVerb("mark", args), tell: (e) => INBOX(e, dir) }); };
  ...
  function chatSend(body = {}) {
    ...
    // Into the transcript, and — after a read of their site failed — a nudge
    // to look again now (lib/customer.mjs heardFrom).
