...
import { nextCards, readStash, patchStash, proposable, onboarded, draftTabs, wantBrowser, gotBrowser } from "./cards.mjs";
import { styleOf } from "./writing.mjs";
import { controlBroker, isHeld } from "./control.mjs";
...
import { listProjects, currentProject, currentDir, createProject, useProject, slug } from "./projects.mjs";
import { isCustomer, customerOf, writeCustomer, looksLikeEmail, siteOf, hostOf as siteHost, say, heard, setCard, chatState, messageOf } from "./chat.mjs";
import { offerFrom, offerCard, saysNothing, list, filesOf, campaignOf, meOf, lookOf, secondPass, lookNote, isFresh, instructionFrom, opportunityText, EVERY_MIN, CAMPAIGN, FIRST_LOOK } from "./quest.mjs";
import { applyRevision } from "./revise.mjs";
import { bansPromotion } from "./rules.mjs";
import { saidNothing } from "./clock.mjs";
...
// `home` is the folder the extension is loaded from (its manifest is at the
// root) — only a server on the person's own machine knows it, and only to tell
// them on the chat page what to load when no Chrome is attached (chatView).
export function engine({ root, base = null, home = null } = {}) {
  ...
  /** A customer's browser is a patient one (lib/browse.mjs): nobody is at a
   *  terminal to read "try again once they grant it", so a permission wall is
   *  a hold — said in the job's own log, and over when the operator has
   *  pressed Allow. Anyone else's browser answers the wall at once, as before. */
  const hostOfOrigin = (origin) => { try { return new URL(origin).host; } catch { return String(origin); } };
  const patientFor = (d, say = null, signal = null) => (isCustomer(d)
    ? { patient: true, ...(signal ? { signal } : {}), ...(say ? { onHold: (w) => say(w.kind === "window" ? "waiting for Chrome's window to be in view" : `waiting for you to allow ${hostOfOrigin(w.origin)} in the extension`) } : {}) }
    : {});
  const J = perDir((d) => jobStore(d, { verbs: (o) => verbs({ root: DATA, dir: d, browse: (x) => browser(LANE, { ...x, ...patientFor(d, o.log) }), ...o }) }));
  ...
  async function loadRuntime(dir = P()) {
    // A second caller waits for the first one's load rather than getting a
    // half-made entry: a customer's first message arrives in the same tick
    // as the switch that starts the load, and a turn taken before the lane
    // is attached answers "no browser here" to the one read that matters.
    if (RUNTIMES.has(dir)) { const had = RUNTIMES.get(dir); await had.ready; return had; }
    const entry = { T: null, why: "the runtime is still loading", inbox: null, ready: null };
  ...
  /** What a tick would read now: watched rooms past their cadence, and
   *  conversations nobody has looked at in twelve hours. ONE notion of due
   *  (0.7.0: "computed, never remembered") — the nudge card, the panel's
   *  Campaigns tab and the scheduler below all ask this. */
  function dueNow(sources = S.sources()) {
    const lr = new Map(S.readJsonl("reads.jsonl").filter((r) => r.source).map((r) => [r.source, r]));
    return {
      // The tick's own rule (lib/verbs.mjs): never read as itself counts from
      // when it was added, which is when its probe read it.
      sources: sources.filter((s) => { const r = lr.get(s.id); const last = r ? Date.parse(r.at) : Date.parse(s.added ?? ""); return !Number.isFinite(last) || Date.now() - last >= s.cadence_min * 60_000; }).length,
  ...
  /* The clock reads by itself (0.12.0). `tick` is the verb that reads what is
   * due and writes the day's digest — the one thing in this product that had
   * to happen on the wall clock and only ever happened when somebody pressed
   * a card. An idle Messaging Quest stayed idle forever, and the digest the
   * CMO is instructed to react to only arrived on the days a human pressed
   * the button.
   *
   * The guards are autoJudge's, one for one, because that function is the
   * house precedent for a verb that starts itself: a model exists (a read
   * that fills a queue nothing can judge is a queue that rots), the verb is
   * not already running, no other read is in flight, something is actually
   * due, and not within a back-off window of the last failure. Two of its
   * own, because this one needs a hand: the operator can switch it off on
   * Settings, and it HOLDS while no browser is on the lane — saying so once,
   * on the deck, rather than every pass into a log nobody opens.
   */
  const TICK_RETRY_MS = 30 * 60_000;
  const TICK_QUIET_MS = 5 * 60_000;     // and never twice inside five minutes
  function autoTick() {
    try {
      const dir = P();
      if (readStash(dir).auto_tick === false) return;      // the off switch, on Settings
      if (!hasModel(dir) || J.busy("tick")) return;
      if (["probe", "sync", "back", "check", "watch", "pull", "add", "scout", "me", "rules"].some((v) => J.busy(v))) return;
      const due = dueNow();
      if (!due.sources && !due.conversations) return;
      // The last tick is the job store's to remember, not ours — a failure
      // costs half an hour, a success five minutes (a conversation stays due
      // for twelve hours whatever happens, so without this the same read
      // would start every minute).
      const last = J.list().find((j) => j.verb === "tick");
      if (last) {
        const at = Date.parse(last.finishedAt ?? last.startedAt);
        if (Number.isFinite(at) && Date.now() - at < (last.status === "error" ? TICK_RETRY_MS : TICK_QUIET_MS)) return;
      }
      if (!CONTROL.attached()) {
        // Held, not failed. The deck's card is the "once": it is computed
        // from this line and disappears by itself when the hand comes back.
        wantBrowser(dir, "the clock, reading what is due");
        return;
      }
      J.start("tick", [], { label: SPAWNABLE.tick });
    } catch (e) {
      console.error(`the clock could not read what is due: ${e?.message ?? e}`);
    }
  }
  const tickClock = setInterval(autoTick, 60_000);
  tickClock.unref?.();
  ...
  const startScout = (siteUrl) => {
    const started = J.run("scout", async (ctl) => {
      ...
      return await scoutSite(P(), siteUrl, ctl, { browse: browser(LANE, { task: "reading your site", project: P(), ...patientFor(P(), ctl.log, ctl.signal) }) });
  ...
  function cardSnapshot() {
    ...
    // The hand came back: the reconnect card's ask is spent, and so is its
    // snooze — the same self-healing shape as the watch above, and the reason
    // the card never has to be dismissed to go away (0.12.0).
    if (CONTROL.attached() && (stash.browser_wanted || stash.browser_later)) stash = gotBrowser(P());
    ...
    const due = dueNow(sources);

    return {
      stash,
      // The lane, for the card that offers the reconnect (0.12.0) — and for
      // the specialist's turn, which is told the same thing.
      ...
      // A customer's project deals only what a hand at this browser can do
      // (lib/cards.mjs nextCards, Stage 1 of the Quest plan).
      customer: isCustomer(P()),
    };
  }
  ...
  async function actCard({ card, action, choice, choices, text, tab }) {
    ...
    if (id === "cmo.propose") {
      ...
      const p = list[0];
      const spend = () => patchStash(P(), { proposals: list.slice(1) });
      if (act !== "start" || !p?.task?.agent) { spend(); if (p && RT()) RT().note("proposal.dismissed", { agent: p.task?.agent, question: p.question }); return { ok: true }; }
      if (!RT()) return { error: "the runtime is not installed — npm run brain" };
      ...
      // A REFUSAL KEEPS THE CARD (0.12.0). Two colleagues are already at work,
      // or the browser is not there: the answer is "not yet", and a card spent
      // on "not yet" makes the operator ask for the proposal again. Only a
      // dismissal and a real start spend it.
      if (r.error) { if (RT()) RT().note("proposal.refused", { agent: p.task.agent, question: p.question, why: r.error }); return { error: r.error }; }
      spend();
    ...
    if (id === "onboard.account") {
      ...
      // "Find it in my browser": no name, and the verb asks the platform's
      // own address who this browser is signed in as.
      if (act === "find") return spawn("me", []);
    ...
    // "Look again" is the client re-reading the deck, which it does after
    // every act anyway; the lane's own state answers, so there is nothing to
    // do here but not lie about it. "Not now" snoozes for half an hour.
    if (id === "work.browser") {
      if (act === "later") { patchStash(P(), { browser_later: new Date().toISOString() }); return { ok: true }; }
      return { ok: true };
    }
  ...
  // Stage 1 of the Quest plan (2026-09-18). A customer is a project with a
  // customer.json (lib/chat.mjs), made by the front page's sign-up — a stub
  // for now: an email, not checked, which Supabase Auth replaces in Stage 2.
  // One customer at a time: signing up makes their project the current one,
  // because the engine's clocks (autoJudge, autoTick) follow the current
  // project, and so does everything the operator's console shows.

  /** Turns in flight per customer directory — the chat's "Quest is
   *  thinking". A count, because two messages sent quickly are two turns
   *  (the strategist queues them) and the first to finish must not clear it. */
  const THINKING = new Map();

  /** Why Quest could not answer, as something the person in the chat can
   *  read. On this machine that person is also the operator, so the fix is
   *  named plainly; in Stage 2 the operator hears it instead. */
  const questDown = (e) => {
    const m = String(e?.message ?? e).split("\n")[0];
    if (/Cannot find (module|package)|ERR_MODULE_NOT_FOUND|no Node on this host/i.test(m)) return "I'm not running on this machine yet — the agent isn't installed (npm run brain).";
    if (/no OpenRouter key/i.test(m)) return "I can't think without a model — there is no OpenRouter key on this machine yet (Settings in the console).";
    return `Something went wrong on my side (${m.slice(0, 160)}). Say it again in a minute.`;
  };
   ...
   * message, and what to tell Quest about the rest. `interpret` is injectable.
   */
  async function applyInstruction(dir, text, interpret = null) {
    const st = readStash(dir);
    if (!st.offer) return null;
    let got;
    try {
      const fn = interpret ?? (await import("./agents.mjs")).interpretInstruction;
      got = instructionFrom(await fn(dir, text));
    } catch (e) {
      console.error(`an instruction could not be read: ${String(e?.message ?? e).split("\n")[0]}`);
      return null;
    }
    if (!got.leave_out.length && !got.places.length) return null;
    const room = (p) => LB().room(p);
  ...
  /** One Quest turn, in the background: the route answers at once, and the
   *  reply lands in the transcript when it is ready — the same place a
   *  heartbeat's note lands, so a reader never needs to know which it was. */
  function quest(dir, text) {
    THINKING.set(dir, (THINKING.get(dir) ?? 0) + 1);
    (async () => {
      try {
        if (host().kind !== "node") throw new Error("no Node on this host");
        await loadRuntime(dir);          // the lane and the inbox, before the first turn needs them
        // A change to who is found is an instruction, not a conversation.
        const applied = await applyInstruction(dir, text);
        if (applied?.said) say(dir, applied.said);
        if (applied?.done) return;
        const { strategist } = await import("../agent/strategist.mjs");
        const { reply } = await strategist(dir, text, "chat", { note: applied?.note ?? "" });
        if (reply && !saidNothing(reply)) say(dir, reply);
      } catch (e) {
        console.error(`Quest could not answer: ${String(e?.message ?? e).split("\n")[0]}`);
        say(dir, { text: questDown(e), error: true });
      } finally {
        const n = (THINKING.get(dir) ?? 1) - 1;
        if (n > 0) THINKING.set(dir, n); else THINKING.delete(dir);
      }
    })();
  }

  /**
   * The front page's sign-up. `text` is what they typed in the big box —
   * their site's address, usually — and it becomes the first message of the
   * chat, so nothing they wrote is asked for twice. The same email again is
   * the same customer, back to their own project.
   */
  function signup(body = {}) {
    const email = String(body.email ?? "").trim().toLowerCase().slice(0, 200);
    if (!looksLikeEmail(email)) return { error: "that does not look like an email address" };
    const said = String(body.text ?? "").trim().slice(0, 4000);
    const url = siteOf(said);
    // A CV instead of a site (slice 4): a PDF, as the data URL the page read
    // it into. The server's JSON limit is 2 MB, so the file is under 1.4 MB.
    let cv = null;
    if (body.file) {
      const data = String(body.file.data ?? "");
      if (!/^data:application\/pdf;base64,[A-Za-z0-9+/=]+$/.test(data)) return { error: "a CV has to be a PDF" };
      if (data.length > 1_900_000) return { error: "that PDF is too large — under 1.4 MB, please" };
      cv = { name: String(body.file.name ?? "cv.pdf").replace(/[^\w .()-]/g, "").trim().slice(0, 80) || "cv.pdf", data };
    }
    // …or their own words: a sentence or two about what they sell, no site.
    const about = !url && said.length >= 20 ? said : null;
    const known = listProjects(DATA).find((p) => customerOf(p.dir)?.email === email);
    let pj;
    if (known) pj = useProject(DATA, known.id);
    else {
      // Named for the site — and for its path when the site is somebody
      // else's (github.com/theic/app), or every customer on one host would be
      // "github-com-7" to the operator who reads this list.
      let named = email.split("@")[0];
      if (url) { try { named = `${siteHost(url)}${new URL(url).pathname.replace(/\/+$/, "")}`; } catch { named = siteHost(url) ?? named; } }
      const base = slug(named) || "customer";
      let id = base;
      for (let n = 2; listProjects(DATA).some((p) => p.id === id); n++) id = `${base}-${n}`.slice(0, 40);
      pj = createProject(DATA, id, { inherit: false });
    }
    if (pj.error) return pj;
    if (cv) writeFileSync(join(pj.dir, "cv.json"), JSON.stringify({ name: cv.name, data: cv.data }));
    const customer = writeCustomer(pj.dir, {
      email, ...(known ? {} : { since: new Date().toISOString() }),
      ...(url ? { url } : {}), ...(cv ? { cv: { name: cv.name, at: new Date().toISOString() } } : {}), ...(about ? { about } : {}),
    });
    afterSwitch();
    const first = [said, cv ? `My CV: ${cv.name}` : ""].filter(Boolean).join("\n\n");
    if (first) { heard(pj.dir, first); quest(pj.dir, first); }
    return { ok: true, id: pj.id, customer };
  }

  /* The site read, by itself (slice 2). On a ten-second clock, for the
   * current customer: a site nobody has read yet is read the moment a
   * browser is on the lane — held while there is none, never failed (the
   * reconnect nudge asks the operator, once, on the deck) — and what the
   * reader understood is posted in the chat as the confirm card. Three reads
   * that open nothing and Quest asks the customer about the address instead
   * of trying for ever. */
  ...
  function autoOffer() {
    try {
      const dir = P();
      if (!isCustomer(dir)) return;
      const st = readStash(dir);
      if (st.offer || st.scout_empty) return;
      const job = st.scoutJob ? J.get(st.scoutJob) : null;
      if (job?.status === "running") return;
      if (job?.status === "ok") {
        const proposal = J.result(st.scoutJob);
        if (proposal && (proposal.one_line || proposal.rule_md)) { postOffer(dir, proposal); return; }
      }
      const c = customerOf(dir) ?? {};
      const url = c.url;
      // No site, but a CV or their own words (slice 4): read here, no browser.
      const material = !url && (c.cv || c.about) ? (c.cv ? "cv" : "about") : null;
      // A read that stopped on what only the person at the machine can lift — a
      // permission wall, or a Chrome window nobody is drawing (the hold in
      // lib/browse.mjs ran out with the operator away) — was not a failed read of
      // the address: it is not counted against it, and it does not wait out ten
      // minutes to start again — the ask is still standing, and the next pass
      // holds on it. Measured 2026-09-19: the wall, Allow pressed, then "Try
      // again" in the chat — and nothing for ten minutes, the chat saying
      // "being read now"; then three reads on a covered window, and Quest asked
      // a founder whether their address was right.
      const wall = job?.status === "error" && isHeld(job.error);
      let tries = Number(st.scout_tries) || 0;
      if (wall && st.scout_walled !== st.scoutJob) { tries = Math.max(0, tries - 1); patchStash(dir, { scout_walled: st.scoutJob, scout_tries: tries }); }
      // A read that failed is not being read: Quest's line about it (agent/strategist.mjs readingSite) goes by this.
      if (job?.status === "error" && st.scout_at) patchStash(dir, { scout_at: null });
      if ((url || material) && tries >= OFFER_TRIES) {
        if (!st.scout_gaveup) {
          patchStash(dir, { scout_gaveup: true });
          say(dir, url
            ? `I couldn't open ${siteHost(url) ?? url} — is that the right address? Paste the right one here, or tell me in a sentence what you sell and who buys it.`
            : `I couldn't make sense of ${material === "cv" ? "your CV" : "that"} — tell me in a sentence or two what you sell and who buys it.`);
        }
        return;
      }
      if ((!url && !material) || !hasModel(dir) || J.busy("scout")) return;
      // What they said since it failed is a nudge (chatSend): they are looking at it, and the
      // ten minutes are for a site nobody is waiting on. The three tries still bound it.
      const nudged = Boolean(job && st.scout_nudge) && Date.parse(st.scout_nudge) > Date.parse(job.finishedAt ?? job.startedAt);
      if (job && !wall && !nudged && Date.now() - Date.parse(job.finishedAt ?? job.startedAt) < OFFER_RETRY_MS) return;
      if (material) {
        const started = J.run("scout", async (ctl) => {
          const { offerFromMaterial } = await import("./agents.mjs");
          const file = c.cv && existsSync(join(dir, "cv.json")) ? JSON.parse(readFileSync(join(dir, "cv.json"), "utf8")) : null;
          return await offerFromMaterial(dir, { text: c.about ?? "", file }, ctl);
        }, { label: material === "cv" ? "Reading your CV" : "Reading what you sell" });
        if (!started.error) patchStash(dir, { scoutJob: started.id, scout_tries: tries + 1, scout_at: new Date().toISOString() });
        return;
      }
      if (!CONTROL.attached()) { wantBrowser(dir, "reading a customer's site"); return; }
      const started = startScout(url);
      // When it started, for Quest's own line about it (agent/strategist.mjs
      // readingSite): the job lives in this process's memory, the stash on disk.
      if (!started.error) patchStash(dir, { scout_tries: tries + 1, scout_at: new Date().toISOString() });
    } catch (e) {
      console.error(`the site read could not start by itself: ${e?.message ?? e}`);
    }
  }

  /* The first look (slice 3). After "Looks right", each community the offer
   * names is taken in turn: its rules page read once in the signed-in
   * browser — a room whose own rules forbid promotion is skipped, and the
   * sentence that forbids it goes into its room file, the same file a human
   * would have answered (lib/rules.mjs: "a human or a browser-lane agent
   * reads the sidebar once") — then one read of a search for the sentence
   * somebody types when they have the problem. The judge that already runs
   * by itself judges what came back; a room that clears the floor is watched
   * a few times a day. One page-turning step at a time: it is one browser. */
  const RULES_TRIES = 2;
  function readRules(dir, place) {
    const label = LB().room(place);
    return J.run("rules", async (ctl) => {
      const b = browser(LANE, { task: `reading ${label}'s rules`, project: dir, ...patientFor(dir, ctl.log, ctl.signal) });
      try {
        const o = await b.open(LB().rulesUrl(place));
        if (o.error) throw new Error(o.error);
        const t = await b.text(20_000);
        if (t.error) throw new Error(t.error);
        // The sentence it sits in — and, on a page whose text has no full stop
        // before the rules list, that starts at the page furniture ("Skip to
        // main content Open navigation…"): keep the last 240 characters.
        const found = bansPromotion(t.text);
        const hit = found && found.length > 240 ? `…${found.slice(-240).replace(/^\S*\s/, "")}` : found;
        const base = existsSync(S.roomPath(place)) ? readFileSync(S.roomPath(place), "utf8") : roomFile(place, null, { label, rulesUrl: LB().rulesUrl(place) });
        const when = new Date().toISOString().slice(0, 10);
        S.writeRoom(place, base.replace(/^promotion_allowed:.*$/mi, `promotion_allowed: ${hit ? "no" : "yes"}`)
          + `\n## Read by Messaging Quest, ${when}\n\n${hit ? `Its rules say: "${hit}"` : "Nothing on its rules page forbids promotion. Correct the line above if a human reading them disagrees."}\n`);
        ctl.log(hit ? `${label} forbids it: "${hit}"` : `${label}'s rules allow it`);
        return { state: hit ? "banned" : "allowed", quote: hit ?? null };
      } finally {
        await b.close();
      }
    }, { label: `Reading ${label}'s rules` });
  }
  ...
  function autoLook() {
    ...
    // The probe and the watch both refuse a campaign that is not on disk; a
    // file lost since "Looks right" must not quietly stop the look.
    ...
    // What finished since the last pass. A job this process no longer holds
    // (a restart) is a step to take again, not a step taken.
    for (const r of rooms) {
      if (r.rules === "reading") {
        const j = r.rulesJob ? J.get(r.rulesJob) : null;
        if (!j) set(r, { rules: "todo", rulesJob: null });
        else if (j.status === "ok") {
          const banned = J.result(r.rulesJob)?.state === "banned";
          set(r, { rules: banned ? "banned" : "allowed", rulesJob: null, ...(!banned && !r.q && phrases.length ? { q: phrases[used++ % phrases.length] } : {}) });
        }
        else if (j.status === "error") set(r, (r.tries ?? 0) + 1 >= RULES_TRIES ? { rules: "failed", rulesJob: null } : { rules: "todo", rulesJob: null, tries: (r.tries ?? 0) + 1 });
      }
      if (r.probe === "running") {
        const j = r.probeJob ? J.get(r.probeJob) : null;
        if (!j) set(r, { probe: "todo", probeJob: null });
        else if (j.status === "ok") set(r, { probe: "done", probeJob: null });
        else if (j.status === "error") set(r, { probe: "failed", probeJob: null });
      }
      ...
      if (r.probe === "done" && r.watch === "todo" && !J.busy("judge")) {
        const tag = `${r.place}:${r.q ?? "new"}`;
        const waiting = S.pending().some((x) => x.probe === tag);
        if (!waiting) {
          const id = tag.toLowerCase();
          runVerb("watch", [r.place, ...(r.q ? ["--q", r.q] : []), "--campaign", CAMPAIGN, "--every", String(EVERY_MIN)]).catch(() => {});
          set(r, { watch: S.sources().some((s) => s.id === id) ? "watched" : "pending" });
        }
      }
      if (r.watch === "pending") set(r, { watch: S.sources().some((s) => s.id === `${r.place}:${r.q ?? "new"}`.toLowerCase()) ? "watched" : "refused" });
    }

    // The next page-turning step, if the browser is free.
    const busy = ["rules", "probe", "scout", "tick", "sync", "check", "back"].some((v) => J.busy(v));
    const next = rooms.find((r) => r.rules === "todo" || (r.rules === "allowed" && r.probe === "todo"));
    if (next && !busy) {
      if (!CONTROL.attached()) wantBrowser(dir, "looking in the communities a customer confirmed");
      else if (next.rules === "todo") {
        const j = readRules(dir, next.place);
        if (!j.error) set(next, { rules: "reading", rulesJob: j.id });
      } else {
        const j = J.start("probe", [next.place, ...(next.q ? ["--q", next.q] : []), "--campaign", CAMPAIGN], { label: `Looking in ${LB().room(next.place)}` });
        if (!j.error) set(next, { probe: "running", probeJob: j.id });
      }
    }
    ...
    if (settled && !st.look.said && !S.pending().length && !J.busy("judge")) {
      // Counted off the verdicts, not the cards: delivery runs a moment after
      // this, so a look that found people has fits here before it has cards.
      const verdicts = S.verdicts();
      const mine = [...S.found().values()].filter((f) => rooms.some((r) => f.probe === `${r.place}:${r.q ?? "new"}`) && isFresh(f));
      const read = mine.length;
      const watched = [...new Set(rooms.filter((r) => r.watch === "watched").map((r) => LB().room(r.place)))];
      ...
      if (again.rooms.length) {
        // Not yet: the same rooms, other ways people put it — said by the
        // status line while it reads, and by the summary if it too finds nobody.
        ...
        if (nobody) {
          const did = read > 0 ? `I read ${read} post${read === 1 ? "" : "s"} from the last week${readIn.length ? ` in ${list(readIn)}` : ""}` : `Nothing from the last week turned up${readIn.length ? ` in ${list(readIn)}` : ""}`;
          say(dir, [
            watched.length
  ...
  /* Opportunities (slice 3): every fit, posted in the chat the moment it is
   * judged — at most two a community and five a day, the pacing the
   * operator's own replies are held to, here holding how often a customer is
   * interrupted — and its reply written after, onto the same card. */
  ...
  const excerpt = (s, n) => { const t = String(s ?? "").replace(/\s+/g, " ").replace(/([a-z0-9€$%)][.!?])([A-Z])/g, "$1 $2").trim(); return t.length > n ? t.slice(0, n - 1) + "…" : t; };
  function postOpportunity(dir, r) {
    const room = (p) => LB().room(p);
    return say(dir, {
  ...
  function autoDeliver() {
    ...
    const today = cards.filter((m) => Date.now() - Date.parse(m.at) < DAY);
    if (today.length < OVERALL_24H) {
      const next = queueRows().find((r) => !had.has(r.id) && isFresh(r) && S.roomState(r.place).state !== "banned"
        && today.filter((m) => m.card.place === r.place).length < PER_ROOM_24H);
      if (next) { postOpportunity(dir, next); INBOX({ type: "opportunity.new", title: next.title ?? "", place: next.place }, dir); return; }
    }
    ...
    if (round) {
      ...
      if (drafts.length) setCard(dir, want.id, { rewriting: null, draft: drafts[0].text, drafts: drafts.map((d) => { const check = results(d.text); return { style: d.style, text: d.text, ...(check.length ? { check } : {}) }; }) });
      return;
    }
    if (J.busy("draft") || !hasModel(dir)) return;
    const failed = lastOf(dir).draft;
    if (failed?.id === want.card.item) {
      if (/declined/.test(failed.error ?? "")) { setCard(dir, want.id, { no_draft: failed.error, rewriting: null }); return; }
      if (Date.now() - failed.at < 10 * 60_000) return;
    }
    startAgentic("draft", rw ? [want.card.item, "--note", rw.note, ...(rw.style ? ["--style", rw.style] : [])] : [want.card.item]);
  }

  /** Everything the customer's side does on its own, on one ten-second
   *  clock, each step on its own guard so one failing cannot stop the rest. */
  function customerClock() {
    for (const [name, fn] of [["the site read", autoOffer], ["the first look", autoLook], ["delivering", autoDeliver]]) {
      try { fn(); } catch (e) { console.error(`${name} could not run by itself: ${e?.message ?? e}`); }
    }
  }
  const offerClock = setInterval(customerClock, 10_000);
  offerClock.unref?.();

  /** The reader's proposal, as the card a customer answers. */
  function postOffer(dir, proposal) {
    ...
    if (saysNothing(offer)) {
      ...
      patchStash(dir, { scoutJob: null, scout_empty: true });
  ...
  function confirmOffer(dir) {
    ...
    const where = offer.places.slice(0, FIRST_LOOK).map((p) => LB().room(p));
    ...
    INBOX({ type: "offer.confirmed", title: offer.one_line, places: offer.places }, dir);
    return { ok: true };
  }

  /** A button on a card in the chat. The card is looked up on the server and
   *  must still be the live one — a customer pressing an old card after it
   *  was replaced gets told, not a stale confirmation. */
  function chatAct(body = {}) {
    const dir = P();
    ...
    if (m.card.kind === "offer") {
      ...
      if (body.action === "confirm") return confirmOffer(dir);
    }
    if (m.card.kind === "opportunity") return actOpportunity(dir, m, String(body.action ?? ""), body);
  ...
  async function actOpportunity(dir, m, action, body = {}) {
    const at = new Date().toISOString();
    const item = m.card.item;
    const tell = (type) => INBOX({ type, title: m.card.title, place: m.card.place, item }, dir);
    if (action === "rewrite") {
    ...
    if (action === "good") { setCard(dir, m.id, { rating: "good", rated_at: at }); tell("opportunity.good"); return { ok: true }; }
    if (action === "bad") {
      setCard(dir, m.id, { rating: "bad", rated_at: at, state: "dismissed" });
      await runVerb("mark", [item, "skip"]).catch(() => {});
      tell("opportunity.bad");
      return { ok: true };
    }
    if (action === "replied") {
      ...
      await runVerb("mark", [item, "sent", "--anyway"]).catch(() => {});
      tell("opportunity.replied");
      return { ok: true };
    }
    return { error: "that is not something this card does" };
  }

  /** What the chat shows, for the current project — null when it is not a
   *  customer's (the operator's own, or nobody has signed up yet). */
  function chatView() {
    const dir = P();
    if (!isCustomer(dir)) return { customer: null };
    return {
      customer: { id: PJ().id, ...customerOf(dir) },
      ...chatState(dir),
      thinking: (THINKING.get(dir) ?? 0) > 0,
      // What the engine is doing for them right now, in its own words — the
      // chat shows it under the messages, so a minute-long read is visibly
      // somebody working rather than a silence.
      working: J.running().map((j) => ({ label: j.label, note: j.note ?? null, done: j.done ?? null, total: j.total ?? null })),
      // Whether a Chrome is attached to read with — the one field here that
      // is the operator's, not the customer's. This server listens on
      // loopback only, so whoever reads this page is at the machine, and is
      // the person who has to open the browser; the hosted agent (Stage 2)
      // tells its operator through a channel of its own and sends no such
      // field. Measured 2026-09-19 on the first run in the owner's own Chrome:
      // the extension was in its default hosted mode, nothing polled this
      // lane, and the chat said "in the queue" for 22 minutes — Quest's words
      // never mention the browser (strategist.mjs), so the page has to.
      // `waiting` is a page having been asked for and refused: nothing is
      // shown before something needs a hand.
      // `allow` is the second thing only the person at the machine can fix: a
      // site the extension has been asked to read and may not yet. The read
      // is held on it (lib/browse.mjs patient), so the page says what to press.
      browser: { attached: CONTROL.attached(), waiting: !CONTROL.attached() && Boolean(readStash(dir).browser_wanted), allow: CONTROL.grantsNeeded(), hidden: CONTROL.hidden(), home },
    };
  }
  ...
  function chatSend(body = {}) {
    ...
    // A word from them after a read failed is a nudge to look again now
    // (autoOffer). "Try again" after it gave up is more than a nudge — the three
    // tries are spent — so it is a fresh round: measured 2026-09-19, said twice,
    // and nothing happened either time.
  ...
  const controlSummary = () => ({
    ...
    // Since when, and what is out of reach (0.12.0): the specialist is told
    // both in its turn, and the reconnect card picks its fix from them.
    since: CONTROL.attachedSince(),
    stranded: CONTROL.stranded(),
  ...
  function panelState() {
    ...
    return {
      ...
      settings: {
        plan: p,
        /** The clock, and whether it is allowed to read by itself (0.12.0).
         *  On by default; someone will want it off, and a machine that opens
         *  tabs on its own should be one switch away from not. */
        clock: {
          auto: stash.auto_tick !== false,
          lane: controlSummary().attached,
          due: dueNow(srcs),
        },
  ...
  async function panelAct(body = {}) {
    ...
    switch (act) {
      ...
      // The clock's off switch. Stored as the exception (`auto_tick: false`)
      // so every directory that never touched it reads as on.
      case "settings.clock": { patchStash(dir, { auto_tick: body.auto === false ? false : null }); return { ok: true }; }
      case "settings.key": {
  ...
  const ROUTES = {
    ...
    /* The customer's side (Stage 1 of the Quest plan): the front page's
     * sign-up, who is signed in, and the chat — the transcript and a message
     * into it. Same JSON-only CSRF boundary as every other POST here. */
    "GET /api/me": async () => ({ status: 200, body: { customer: isCustomer(P()) ? { id: PJ().id, ...customerOf(P()) } : null } }),
    "POST /api/signup": async (q, body) => { const out = signup(body ?? {}); return { status: out.error ? 400 : 200, body: out }; },
    "GET /api/chat": async () => ({ status: 200, body: chatView() }),
    "POST /api/chat": async (q, body) => { const out = chatSend(body ?? {}); return { status: out.error ? (out.status ?? 400) : 200, body: out }; },
    "POST /api/chat/act": async (q, body) => { const out = await chatAct(body ?? {}); return { status: out.error ? (out.status ?? 400) : 200, body: out }; },
  ...
  function deck(instance = null) {
    ...
    return {
      ...
      brain: Boolean(RT()),
      // Stage 1 of the Quest plan: when the current project is a customer's,
      // this browser is Quest's hands and the panel shows the worker, not
      // the operator's deck (extension/sidepanel.js).
      customer: isCustomer(P()) ? (({ email, url }) => ({ email: email ?? null, url: url ?? null }))(customerOf(P()) ?? {}) : null,
    };
  }
  ...
  const stop = () => { clearInterval(sweeper); clearInterval(judgeClock); clearInterval(tickClock); clearInterval(offerClock); };

  return {
    DATA, P, PJ, LB, S, J, CONTROL, LANE, RT, WHY, INBOX, loadRuntime, afterSwitch, acct, who, queueRows,
    SPAWNABLE, AGENTIC, startAgentic, autoJudge, autoTick, dueNow, SELF, startScout, cardSnapshot, actCard,
    projectSummary, switchProject, controlSummary, panelState, panelAct, runVerb, capture,
    signup, chatView, chatSend, chatAct, quest, applyInstruction, autoOffer, postOffer, confirmOffer, autoLook, autoDeliver, customerClock,
