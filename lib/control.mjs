...
//
// The read lane (lib/relay.mjs) hands the browser a URL and gets bytes back;
// it is GET-only by construction and stays that way. This is the SECOND
// protocol the extension speaks (PLAN.md, 2026-09-03): a browser toolkit with
// the reach of Claude in Chrome — open a tab, navigate, read the accessibility
// tree, find, scroll, screenshot, and (behind a grant nobody holds in
// milestone 1) click and type. Same long-poll transport as the relay, so the
// heart stays zero-dependency: no WebSocket server, one HTTP server.
//
...
//
// Two halves in one file, like the relay: the BROKER lives in the dashboard
...
export const FAMILIES = ["read", "click", "type"];

/** Which family one call needs, or null when the call is not a toolkit call. */
export function familyOf(tool, input = {}) {
 ...
 */
export function controlBroker({ ttlMs = 60_000, cap = 32, paceMs = 6_000, leaseTtlMs = 30 * 60_000, onEvent = null } = {}) {
  ...
  const queue = (lease, tool, input, tabId) =>
    new Promise((resolve) => {
      ...
      jobs.set(id, { id, lease: lease?.id ?? null, tabId: tabId ?? null, tool, input, resolve, timer, claimed: false });
      wake();
    });
  ...
  const shape = (l) => ({
    id: l.id, task: l.task, tabs: [...l.tabs], group: l.group, window: l.window,
  ...
  const api = {
    ...
    async lease({ task, url } = {}) {
      ...
      const lease = { id, task: String(task ?? "").slice(0, 80), tabs: [], group: null, window: null, url: u,
        openedAt: new Date().toISOString(), lastAt: Date.now(), released: false, grant: null, calls: 0 };
      leases.set(id, lease);
      await sleep(reserve(u));
      const ans = await queue(lease, "lease", { url: u });
      ...
      emit("lease.opened", { lease: id, task: lease.task, tabId: lease.tabs[0], url: u });
      return { id, tabId: lease.tabs[0], groupId: lease.group };
    },
    ...
    async act(leaseId, tool, input = {}, { grants = ["read"] } = {}) {
      ...
      if (!p.ok) { emit("call.refused", { lease: lease.id, tool, why: p.why }); return { error: p.why, refused: true }; }
      ...
      if (ans?.error === "not_granted") {
        ...
        emit("grant.needed", { lease: lease.id, origin: lease.grant });
      ...
      if (ans?.refused) emit("click.refused", { lease: lease.id, why: ans.error });
      return ans;
    },
    ...
    async release(leaseId) {
      ...
      emit("lease.released", { lease: lease.id, task: lease.task });
    ...
    /** What the browser should do right now. Long-polls like the relay. */
    async claim(waitMs = 0) {
      lastPoll = Date.now();
      const take = () => {
        const out = [];
        for (const j of jobs.values()) if (!j.claimed) { j.claimed = true; out.push({ id: j.id, lease: j.lease, tabId: j.tabId, tool: j.tool, input: j.input }); }
        return out;
      };
      let out = take();
    ...
    adopt({ id, task, tabs, url } = {}) {
      const key = String(id ?? "");
      if (!key || leases.has(key)) return leases.get(key) ? shape(leases.get(key)) : null;
      const lease = { id: key, task: String(task ?? "").slice(0, 80), tabs: (tabs ?? []).map(Number).filter(Number.isFinite), group: null, window: null, url: String(url ?? ""),
        openedAt: new Date().toISOString(), lastAt: Date.now(), released: false, grant: null, calls: 0, adopted: true };
...
export function controlClient(base) {
  ...
  return {
    lease: (task, url) => post("/api/control/lease", { task, url }),
