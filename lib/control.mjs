...
/**
 * What a browser tool says when the lane is DARK (0.12.0). The browser is a
 * tool, and tools disconnect: Chrome is closed, the profile was switched, the
 * side panel was never opened in this one. Before this, a call in that state
 * waited out the job's whole timeout and came back "navigate unanswered" or
 * "control lane unreachable: timeout" — true, unusable, and indistinguishable
 * from a bug. So the sentence is typed (`detached: true`) and carries the next
 * step, because an agent relays it from the text alone; and it says what still
 * works, because the answer to a missing hand is to do the other half, not to
 * stop. The lane's other absence — a tab whose own Chrome profile has left,
 * which no other profile can reach (0.9.2) — is `act`'s own sentence, below.
 */
export const LANE_DARK = "Nothing can be read or opened until a browser is back on the lane. The operator has to open Chrome with the Messaging Quest extension loaded and open its side panel once; the panel is what puts a profile on the lane. Everything that does not need a page still works: the store, the verdicts, the drafts already written, the campaigns, this conversation.";
export const laneDark = (why = LANE_DARK) => ({ error: why, detached: true });

/**
 * The other spelling of a site's address, when it has one. To Chrome `www.` and
 * the bare name are two origins, and a permission for one is not one for the
 * other; a site that sends the bare name to www (vocavela.live, measured
 * 2026-09-19: opened at the address the customer typed, landed on www.,
 * "no permission" for a site the operator believed they had allowed) makes
 * every first read a second ask. Only those two: `app.acme.com` has no twin,
 * and nor has an address with a port or a bare IP.
 */
export function siteTwin(origin) {
  let u;
  try { u = new URL(String(origin ?? "").replace(/\/\*?$/, "")); } catch { return null; }
  const h = u.hostname;
  if (!/^https?:$/.test(u.protocol) || u.port || h === "localhost" || h.includes(":") || /^[\d.]+$/.test(h)) return null;
  if (h.startsWith("www.")) return h.slice(4).includes(".") ? `${u.protocol}//${h.slice(4)}` : null;
  return h.split(".").length === 2 ? `${u.protocol}//www.${h}` : null;
}

/**
 * What only the person at the machine can lift. A read that stopped on one of
 * these was not a failed read of the address: the site is not allowed yet, or
 * Chrome is not drawing the window the tab is in (a covered or minimised window
 * gets no page — extension/control.js front()). A customer's browser waits on
 * them (lib/browse.mjs patient), and the clock never counts them against the
 * address. Measured 2026-09-19 in the owner's own Chrome: the wall, then
 * three reads in a row that stopped on a covered window, and Quest asking a
 * founder whether their address was right.
 */
export const isHidden = (error) => /window is not on screen/i.test(String(error ?? ""));
export const isHeld = (error) => /no permission for/i.test(String(error ?? "")) || isHidden(error);
...
export function controlBroker({ ttlMs = 60_000, cap = 32, paceMs = 6_000, leaseTtlMs = 30 * 60_000, silentMs = 90_000, freshMs = 30_000, panelMs = 60_000, onEvent = null } = {}) {
  ...
  let hiddenAt = 0;          // the last time a read answered "the window is not on screen" — until one reads
  ...
  const api = {
    ...
    async act(leaseId, tool, input = {}, { grants = ["read"] } = {}) {
      ...
      lease.lastAt = Date.now();
      if (isHidden(ans?.error)) hiddenAt = Date.now();
      else if (ans && !ans.error) hiddenAt = 0;
    ...
    /** Is a read waiting on Chrome to draw its window? True while the last
     *  answer said so, recently — a page's strip says what to do. */
    hidden: () => hiddenAt > 0 && Date.now() - hiddenAt < 30_000,
    ...
    /** Since when — the earliest of the instances that are on the lane now,
     *  so "attached, since 20 minutes ago" is a fact and not a memory. Null
     *  when nothing is attached. */
    attachedSince: () => {
      const firsts = [...pollers.entries()].filter(([id]) => alive(id)).map(([, p]) => p.first);
      return firsts.length ? new Date(Math.min(...firsts)).toISOString() : null;
    },
    /** Tabs nobody can reach any more: leases bound to a Chrome profile that
     *  left the lane (0.9.2). A count, for the card that says which fix. */
    stranded: () => [...leases.values()].filter((l) => l.instance !== null && !alive(l.instance)).length,
    pending: () => jobs.size,
  };
  return api;
}
...
export function controlClient(base) {
  ...
  return {
    ...
    /** The sites the extension has been asked to allow and has not yet — what a
     *  patient browser (lib/browse.mjs) waits on. Empty when unreachable. */
    grants: async () => {
      try {
        const res = await fetch(`${root}/api/control/leases`, { signal: AbortSignal.timeout(5000) });
...
export function laneOf(broker) {
  return {
    ...
    grants: async () => broker.grantsNeeded(),
  };
}
