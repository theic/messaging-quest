...
/**
 * A FIRST MESSAGE UNDER A CAMPAIGN THAT DISCLOSES (lib/campaigns.mjs,
 * mention: disclosed) — the one lift of the opener's rule a campaign may ask
 * for, and only in the disclosed form. Added 2026-09-04 for the campaign the
 * operator wanted to dogfood: an honest comment that says most replies the
 * person sees are generated, that this is itself the answer to their
 * question, that the operator built a tool for exactly it, and asks for
 * their feedback. That is a disclosed mention on a first message, by
 * design — so the rule set exists, and it is narrower than the conversation
 * rules: named once, as theirs, no link unless asked, and the help first.
 */
const DISCLOSED_RULES = [
    "This is a FIRST MESSAGE, and this campaign allows what the house rule forbids: naming what the user built. Allowed is not required — do it only where it answers what they asked, and never as the opening line.",
    "SAY IT IS YOURS, in one plain clause: 'i built X for this' / 'i made a small tool that does that'. Never recommend it as though you were a neutral passer-by — that is undisclosed advertising, it is against the rules of most communities, and readers spot it far more often than people writing it believe.",
    "X is what the USER built — the thing named under 'what you can honestly say about yourself'. The product in the person's own post is THEIRS: never say you built, made or run it, and never borrow its name. If nothing of the user's is named there, there is nothing to disclose — answer them as help only, with no first-person claim at all.",
    "Once. No feature list, no pitch, no marketing adjectives, no 'DM me', no 'happy to help further'. A link only if they asked for one; the name is enough, anyone interested will search it.",
    "Lead with something real for THEM — the concrete first step, the specific gotcha, the number, the two-line fix. The mention rides on the help; it never replaces it.",
    "If the honest reply to this person has no place for the mention, leave it out entirely and answer them. A reply with a mention wedged in is the thing that gets a person ignored for good.",
    "One small concrete question about their setup is still the best ending — a real question you would want the answer to, not a hook.",
];
/**
...
export function signalWritingRules(ctx) {
    ...
    // Three stages: an opener (sells nothing), a disclosed opener (a campaign
    // lifted the rule, in the disclosed form only), a conversation (they
    // wrote back). The stage is read off the facts, never configured loosely.
    const stage = ["opener", "disclosed", "conversation"].includes(ctx.stage) ? ctx.stage : "opener";
    ...
    return [
        ...
        problem?.trim()
            ? opener
                ...
                : `The problem they fix, in a sufferer's words: "${problem.trim().replace(/[.\s]+$/, "")}". This post got here because it is that problem — so the reply ${stage === "disclosed" ? "may connect the two, once, plainly, as yours" : "has to connect the two out loud"}.`
            : "",
        ...
        style?.trim()
            ...
            : "",
        "",
        ...(opener ? OPENER_RULES : stage === "disclosed" ? DISCLOSED_RULES : CONVERSATION_RULES[intent] ?? CONVERSATION_RULES.leads),
        "",
...
/**
 * THREE DRAFTS, ALWAYS (0.8.0). The writer used to be asked for "up to
 * three options differing by move", and it was allowed to send one; the
 * server kept the first and the other two lived in a job log nobody opened.
 * The operator asked for what a chat product does: three tabs, every time,
 * each a different way of answering, and a comment box on any of them that
 * writes a new round. So the moves are NAMED here, once, and every surface
 * reads the same list — the writer is told which three to write, the card
 * shows them as tabs under these labels, the save records which was posted.
 *
 * They differ by MOVE, not by tone: the tone is the measured voice's (and a
 * campaign's `## Voice` lays over it), and three tones of one sentence is the
 * thing this replaces. A style is what the reply DOES for the person.
 */
export const STYLES = [
  {
    id: "straight", label: "Straight",
    what: "The answer and nothing else: the fix, the number, the doc section, in two to four sentences. The shortest of the three, and it stops when the useful part stops.",
  },
  {
    id: "deeper", label: "Deeper",
    what: "Answers what is behind the question: the thing they will hit next, the step they skipped, the trade-off they have not seen. The longest of the three — and still a comment in a thread, not a post of your own.",
  },
  {
    id: "ask", label: "Ask back",
    what: "One real thing, then a concrete question about their setup that you actually want answered. The shape most likely to get a reply — which is where the rest of this goes.",
  },
];

/** A draft the operator typed or pasted themselves (the MCP tool, `--save`
 *  with plain text) is labelled as theirs, not as one of the three. */
export const YOURS = { id: "yours", label: "Yours", what: "Written by you." };

export const styleOf = (id) => {
  const k = String(id ?? "").trim().toLowerCase();
  return STYLES.find((s) => s.id === k) ?? (k === YOURS.id ? YOURS : null);
};
export const styleLabel = (id) => styleOf(id)?.label ?? String(id ?? "");

/**
 * The instruction block the writer is handed, by `mq draft` and by the
 * on-page prompt alike: the three styles by name, the ceiling, and — on a
 * rewrite — what the operator said about the last round and which draft
 * they were looking at when they said it.
 */
export function draftsBlock({ ceiling = 900, note = "", prior = null, style = null } = {}) {
  const lines = [
    `## Three drafts, one per style — every time`,
    ``,
    `Write THREE replies, one for each style below, in this order. They differ by what they DO for this person, never by tone — the voice rules above decide how each sounds, and all three sound like the same person. Three ways of saying one sentence is the failure this replaces; if the three read alike, rewrite them until they do not.`,
    ``,
    ...STYLES.map((s) => `- ${s.id} — ${s.label}. ${s.what}`),
    ``,
    `Each under ${ceiling} characters. If there is genuinely nothing honest to say without naming what the operator sells, say so as a no-fit instead of padding three drafts.`,
  ];
  if (note) {
    const which = styleOf(style);
    lines.push(
      ``,
      `## What the operator said about the last round — this outranks everything above except the refusals`,
      ``,
      note,
      ``,
      which ? `They were looking at the ${which.label.toUpperCase()} draft when they said it.` : `They were looking at the draft below when they said it.`,
    );
    if (prior) lines.push(``, `--- rejected ---`, String(prior).trim(), `--- end ---`);
    lines.push(``, `Write all three again, doing what the note asks in each — the one they commented on most of all. Do not lightly edit the rejected draft; write a different reply that does what they asked.`);
  }
  return lines.join("\n");
}
