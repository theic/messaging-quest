...
function nudgeCards(snap) {
  ...
  /* The browser is a tool, and tools disconnect (0.12.0). While the lane is
     dark, "Read what is due" is a button that cannot fire — so this card
     takes its place and says which fix instead: nothing on the lane at all,
     or tabs stranded on a Chrome profile that left (the 0.9.2 case). It is
     dealt only when something actually wanted a page — the clock holding, a
     specialist's read, a colleague opening a tab — or when a read is due,
     and only when the lane SAYS it is dark: a snapshot that carries no lane
     does not know, and a card that claims the browser is gone because
     nobody asked would be the worst kind of wrong. */
  ...
  if (!dark && n > 0 && !due.running && !snoozed) {
    const parts = [];
