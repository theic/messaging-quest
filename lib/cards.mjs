 ...
 */
export const CAMPAIGN_STEPS = ["idea", "fit", "mention", "room", "phrase"];
...
function campaignCards(snap) {
  ...
  if (!d?.name || !d.idea) {
    // No draft in the walk — but a campaign's probe may be in flight or
    // landed on a project whose onboarding is long done.
    return snap.stash?.probe?.campaign ? probeCards(snap, L) : [];
  }
...
function onboardingCards(snap) {
  ...
  if (needFiles && !stash.manual && scoutState === "none" && stash.url && !snap.hasModel) {
    cards.push({
      ...
      help: "Paste an OpenRouter key — the free models need one too, and cost nothing. It is stored in .mq/openrouter.key on this machine and never leaves it except to call the model. Or run a model on this machine: pick Local on Settings, then come back.",
      ...
      links: [{ href: "/settings", label: "Run it locally instead" }],
    });
  }
...
function conversationCards(snap) {
  ...
  if (c.draft) {
    return [{
      ...
      field: { placeholder: "", value: c.draft.text, multiline: true },
      ...
      data: { url: c.url, draft: c.draft.text, id: c.id, client: "insert", flags: c.draft.flags ?? null, insert: c.composer ?? null, submit: L.submit, turn: c.turn },
    }];
  }
  return [{
    id: `work.turn.${c.id}`, kind: "work.turn", eyebrow, question, help,
...
function rewriteCards(snap) {
  ...
  return [{
    ...
    eyebrow: `${r.who ?? "the draft"} · rewrite`,
    question: "What should change?",
    help: `The draft you were looking at:\n\n${trim(r.prior, 700)}`,
    field: { placeholder: "e.g. less salesy — lead with the thing that worked for me, drop the offer", value: "", multiline: true },
    primary: { id: "rewrite", label: "Rewrite it" },
    secondary: { id: "keep", label: "Keep the draft" },
  }];
}
    ...
    if (it.draft) {
      cards.push({
        ...
        data: { url: it.url, draft: it.draft.text, id: it.id, client: "insert",
          flags: it.draft.flags ?? null, insert: it.composer ?? null, submit: L.submit },
      });
    } else {
      cards.push({
