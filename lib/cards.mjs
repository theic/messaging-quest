...
export function nextCards(snap) {
  ...
  // for a person waiting"). Then the people, then what workers brought back,
  // then the specialist's own questions, then setup, then nudges.
  const deck = [
    ...grantCards(snap), ...taskCards(snap), ...triageCards(snap), ...taskResultCards(snap),
...
export function questionCards(questions, answers = {}, { prefix, kind = "ask", eyebrow, image, data, actions, links } = {}) {
  ...
  return {
    ...
    // A field when asked for, or when there is nothing to press — an empty
    // box is never the ONLY way to answer unless the answer cannot be listed.
    field: q.field || !choices ? { placeholder: trim(q.field?.placeholder, 80), value: "", multiline: Boolean(q.field?.multiline) } : undefined,
