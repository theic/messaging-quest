...
import { existsSync, readFileSync, writeFileSync, join } from "./fs.mjs";
...
import { siteTwin } from "./control.mjs";
...
/** Something needed a page and the browser was not there (0.12.0). Written
 *  by whoever wanted it — the scheduler holding a tick, a specialist's read,
 *  a colleague opening a tab — and read by the card below, so the deck can
 *  offer the fix instead of leaving a failure in a log. In the stash because
 *  it is exactly what the stash is for: losing it costs a re-ask, never
 *  data, and it must survive the restart that a reconnect often involves. */
export const wantBrowser = (dir, what) => patchStash(dir, { browser_wanted: { at: new Date().toISOString(), what: trim(what, 120) } });

/** …and the lane came back: the ask is spent, and so is its snooze. */
export const gotBrowser = (dir) => {
  const s = readStash(dir);
  return s.browser_wanted || s.browser_later ? patchStash(dir, { browser_wanted: null, browser_later: null }) : s;
};
...
export function nextCards(snap) {
  ...
  // A CUSTOMER'S project (Stage 1 of the Quest plan, lib/chat.mjs): the
  // customer answers in the chat, never on this deck, and the operator's own
  // setup — account, voice, rooms — is not theirs to walk. What is left for
  // the operator here is only what a hand at this browser can do: allow a
  // site, bring the browser back. Nothing else, and no quiet card either.
  if (snap.customer) return [...grantCards(snap), ...browserCards(snap)];
  ...
  // 0.12.0: a dark lane is a NUDGE, not an emergency — it deals with the
  // clock's own card, down where "reads are due" lives. The browser is every
  // hand this product has, so the temptation was to put it first; a person
  // who wrote back still outranks it, and everything on the deck above the
  // nudges can be answered without a page.
  const deck = [
...
function grantCards(snap) {
  return (snap.grants ?? []).slice(0, 1).map((origin) => {
    ...
    // one to the other: one click, one Chrome dialog, both (lib/control.mjs).
    const also = siteTwin(origin);
    const alsoHost = also ? also.replace(/^https?:\/\//, "") : null;
    return {
      id: `grant.${origin}`, kind: "grant.ask",
      eyebrow: "Your browser",
      question: `Allow the extension to read ${host}?`,
      help: `A task opened ${host} in its tab and this browser has not allowed the extension there yet. Chrome asks you once, in the click${alsoHost ? ` — for ${alsoHost} too, since a site often sends one address to the other` : ""}; nothing is read on that site until you do.`,
      data: { origin, ...(also ? { also } : {}) },
...
function onboardingCards(snap) {
  ...
  /* 1. Who you are on the platform. First because sync/check need it, and
     because it is the cheapest possible first question. Cheaper still when
     the platform can be asked instead of the person: where it declares an
     address that redirects to your own profile, the primary button opens it
     on the lane and reads the handle off where it landed, and the field is
     the fallback rather than the ask. Skippable honestly either way — the
     finding half works with no account; only the visibility half reads one. */
  if (!snap.account?.name && !stash.account_skipped) {
    const findable = Boolean(snap.platform?.whoami);
    cards.push({
      id: "onboard.account", kind: "onboard.account",
      question: L.account.question,
      help: findable
        ? `${L.account.help} You should not have to type it: this opens your own profile in a tab of this browser and takes the name off the address.`
        : L.account.help,
      field: { placeholder: L.account.placeholder, value: "" },
      ...(findable
        ? { primary: { id: "find", label: "Find it in my browser" }, actions: [{ id: "save", label: "Use what I typed" }] }
...
/** The judge runs by itself (0.9.1), so this card is mostly a wait: what
 *  is being judged, while it is. It is a button only when the judge cannot
 *  start on its own — no model — or failed last time, and then it says why. */
const judgeCard = (snap) => {
  const posts = `${snap.pendingCount} post${snap.pendingCount === 1 ? "" : "s"}`;
  if (snap.judging) {
    return {
      id: "work.judge", kind: "work.judge.wait",
      question: `Judging ${posts}.`,
      help: "Started by itself — each is read against rule.md and nothing else. Progress at the top; the people it lets through deal here next.",
      primary: { id: "wait", label: "Judging…" },
    };
  }
  return {
    id: "work.judge", kind: "work.judge",
    question: `${posts} waiting on a verdict.`,
    help: !snap.hasModel
      ? "No model key here — judge them from your assistant over MCP, or add a key on Settings."
      : snap.judgeFailed
        ? `The last run failed: ${trim(snap.judgeFailed, 200)}. It tries again by itself in a few minutes — or now.`
        : "Judging starts by itself in a moment — each is read against rule.md and nothing else.",
    primary: !snap.hasModel ? { id: "open", label: "Open Settings" } : { id: "judge", label: snap.judgeFailed ? "Try again now" : "Judge them now" },
...
function conversationCards(snap) {
  ...
  if (c.draft) {
    ...
    return [{
      ...
      // Where it belongs (0.9.1): under THEIR comment, not in the thread's box.
      data: { url: c.url, draft: tabs[0]?.value ?? c.draft.text, id: c.id, client: "insert", flags: c.draft.flags ?? null, insert: { ...(c.composer ?? {}), target: "comment" }, submit: L.submit, turn: c.turn, round: c.draft.round ?? 1 },
    }];
  }
  const writing = snap.drafting === c.id;
  const failed = snap.draftFailed?.id === c.id ? snap.draftFailed.error : null;
  return [{
    id: `work.turn.${c.id}`, kind: writing ? "work.turn.wait" : "work.turn", eyebrow, question,
    help: `${help}${writing ? "\n\n— the writer is on it: three drafts, a minute or two on the free plan. Progress at the top." : failed ? `\n\n— the last attempt: ${trim(failed, 200)}. Press again to try once more.` : ""}`,
    primary: writing ? { id: "wait", label: "Writing…" } : snap.hasModel ? { id: "draft", label: failed ? "Write it again" : "Write the reply" } : { id: "open", label: "Open the thread" },
    secondary: writing ? undefined : { id: "skip", label: "Let it go" },
    actions: writing ? undefined : [{ id: "posted", label: "I answered it myself" }],
  ...
  for (const it of snap.queue.slice(0, 3)) {
    if (it.draft) {
      ...
      cards.push({
        ...
        help: `${trim(it.body, 360)}\n\n— judged fit: ${it.why ?? ""}${it.readyState === "not ready" && it.readyWhy ? `\n\n— not ready here: ${trim(it.readyWhy, 220)}` : ""}${snap.drafting === it.id ? "\n\n— the writer is rewriting these now; the new round replaces them when it lands." : ""}`,
        ...
        // extension holds no platform words of its own — plus where this
        // reply belongs (0.9.1): a comment on their post goes into the
        // thread's own box, never under the first comment.
        data: { url: it.url, draft: tabs[0]?.value ?? it.draft.text, id: it.id, client: "insert",
          flags: it.draft.flags ?? null, insert: { ...(it.composer ?? {}), target: "post" }, submit: L.submit, round: it.draft.round ?? 1 },
      });
    } else {
      // Being written (0.9.1): the card waits and says so, rather than
      // offering the button again; failed last: the reason is on the card
      // and the button offers once more.
      const writing = snap.drafting === it.id;
      const failed = snap.draftFailed?.id === it.id ? snap.draftFailed.error : null;
      cards.push({
        id: `work.draft.${it.id}`, kind: writing ? "work.draft.wait" : "work.draft",
        eyebrow: `${L.room(it.place)} · ${it.author ?? "?"}${it.campaign ? ` · ${trim(it.campaign, 30)}` : ""}`,
        question: it.title || "Somebody worth answering.",
        help: `${trim(it.body, 360)}\n\n— judged fit: ${it.why ?? ""}${writing
          ? "\n\n— the writer is on it: three drafts, a minute or two on the free plan. Progress at the top."
          : failed ? `\n\n— the last attempt: ${trim(failed, 200)}. Press again to try once more.` : ""}`,
        primary: writing ? { id: "wait", label: "Writing…" } : snap.hasModel ? { id: "draft", label: failed ? "Write it again" : "Write the draft" } : { id: "skip", label: "Not this one" },
        secondary: !writing && snap.hasModel ? { id: "skip", label: "Not this one" } : undefined,
...
/* The browser is a tool, and tools disconnect (0.12.0). While the lane is
   dark, "Read what is due" is a button that cannot fire — so this card takes
   its place and says which fix instead: nothing on the lane at all, or tabs
   stranded on a Chrome profile that left (the 0.9.2 case). It is dealt only
   when something actually wanted a page — the clock holding, a specialist's
   read, a colleague opening a tab, a customer's read — or when a read is due,
   and only when the lane SAYS it is dark: a snapshot that carries no lane
   does not know, and a card that claims the browser is gone because nobody
   asked would be the worst kind of wrong. One card or none, as a list. */
function browserCards(snap) {
  const stash = snap.stash ?? {};
  const due = snap.due ?? { sources: 0, conversations: 0 };
  const n = (due.sources ?? 0) + (due.conversations ?? 0);
  const lane = snap.control ?? {};
  const wanted = stash.browser_wanted ?? null;
  const dark = lane.attached === false && (wanted || n > 0)
    && !(stash.browser_later && Date.now() - Date.parse(stash.browser_later) < 30 * 60_000);
  if (!dark) return [];
  return [{
    id: "work.browser", kind: "work.browser",
    eyebrow: "Your browser",
    question: "No browser is connected.",
    help: `${wanted?.what ? `Something wanted a page and there was none to open: ${trim(wanted.what, 120)}.` : `${n} read${n === 1 ? " is" : "s are"} due and there is nowhere to read.`} ${lane.stranded
      ? "The tabs a task opened belong to a Chrome profile that has left the lane — nobody can reach them from here. Open the side panel in the profile you are actually working in; the task opens a fresh tab there."
      : "Open Chrome with the Messaging Quest extension loaded and open its side panel once — the panel is what puts a profile on the lane."}\n\nEverything that needs no page carries on: the verdicts against your rule, the drafts already written, the campaigns, and your specialist.`,
    primary: { id: "again", label: "Look again" },
    secondary: { id: "later", label: "Not now" },
  }];
}
...
function nudgeCards(snap) {
  ...
  /* The clock (0.7.0). What is due is computed, not remembered: watched
     ...
     From 0.12.0 the clock reads by itself, so this card is no longer the
     only place a tick runs from — it is the one place the operator runs one
     EARLY, and the one honest answer when the clock is holding. "later"
     snoozes it. */
  ...
  // The reconnect card takes the due card's place while the lane is dark
  // (browserCards, above).
  const dark = browserCards(snap);
  cards.push(...dark);

  if (!dark.length && n > 0 && !due.running && !snoozed) {
    const parts = [];
