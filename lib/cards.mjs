...
export function nextCards(snap) {
  const deck = [...onboardingCards(snap), ...workCards(snap)];
...
function onboardingCards(snap) {
  ...
  if (needFiles && !stash.manual && scoutState === "none" && stash.url && !snap.hasKey) {
    cards.push({
      ...
      help: "Paste an OpenRouter key. It is stored in .earshot/openrouter.key on this machine and never leaves it except to call the model.",
  ...
  if (memoryDone && snap.sources.length === 0) {
    if (!stash.probe?.place) {
      ...
      const read = snap.probe?.last?.read ?? 0;
      const rate = snap.probe?.fitRate; // null until judged
      if (snap.pendingCount > 0) {
        cards.push(judgeCard(snap, `r/${stash.probe.place} came back with ${read} post${read === 1 ? "" : "s"}.`));
...
function workCards(snap) {
  ...
  /* Nudges, in the order they start to matter. Each is dismissible and stays
     dismissed — a card that comes back after "Later" is a nag, not a deck. */
  ...
  /* The honest empty state. Only reachable when the machine is genuinely on,
     which is what makes "nothing worth your name" a fact and not a shrug. */
  if (!cards.length) {
    cards.push({
