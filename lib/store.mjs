...
export function store(dir, onError = (m) => { throw new Error(m); }) {
  ...
  return {
    ...
    /** The sources whose cadence is up — one notion of due, for the tick,
     *  the panel and the cloud's clock alike ("computed, never remembered").
     *  Never read counts from when it was added: the probe that earned the
     *  watch read that very page moments before (2026-09-18: a fresh watch
     *  was read again a minute later). A source with neither is due now. */
    due(now = Date.now(), list = null) {
      const lr = new Map(readJsonl("reads.jsonl").filter((r) => r.source).map((r) => [r.source, r]));
      return (list ?? this.sources()).filter((s) => {
        const r = lr.get(s.id);
        const last = r ? Date.parse(r.at) : Date.parse(s.added ?? "");
        return !Number.isFinite(last) || now - last >= s.cadence_min * 60_000;
      });
    },
