 ...
 */
export const page = ({ path, title, body, who = "", refresh = 0, banner = "" }) => `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>${esc(title)} — earshot</title>${refresh ? `<meta http-equiv="refresh" content="${refresh}">` : ""}
<style>${CSS}</style></head><body>
<header><b>earshot</b><span class="who">${esc(who)}</span>
...
<nav>${NAV.map((group) => group.map(([h, t]) =>
  `<a class="${h === path ? "on" : ""}" href="${h}">${t}</a>`).join("")).join('<span class="sep"></span>')}</nav>
${banner}
<main>${body}</main>
...
export const setupBanner = (progress) => {
  ...
  earshot judges every post against what you have told it you sell.
